...
Use the [client walkthrough and feedback sheet](REVIEW.md) for a short review. The portable ZIP includes the public project layout, its root [file manifest](../../release-manifest.json), shared references, and a Node preview helper.
...
From the repository or extracted ZIP root, run `node preview.mjs`, then open `http://127.0.0.1:4173/finishedmockups/`. No package installation is needed. Use HTTP; opening HTML directly with file: does not reliably support modules and JSON requests.
