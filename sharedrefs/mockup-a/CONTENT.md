...
The following commands require the developer's ignored `/tools/` folder and pinned test dependencies. Those files are local and are not included in a fresh clone. Run the app with `node preview.mjs` from the repository root.
