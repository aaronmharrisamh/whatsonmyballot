...
export function mountPaper(viewport,paper,saved,onChange,onView=()=>{},onSnap=()=>{}){
 let state={mode:'free',...saved},fit=1,focused=null,snapEnabled=false,disposed=false,frame=0,tween=null,wheelTimer=0;
 ...
 function bounds(s){
  const sc=fit*s.zoom,w=paper.offsetWidth*sc,h=paper.offsetHeight*sc,pad=32;
  s.x=w<=vw?(vw-w)/2:clamp(s.x,vw-w-pad,pad);
  const minY=Math.min(vh-h-pad,focused?-position(focused).top*sc:0);
  s.y=h<=vh?(vh-h)/2:clamp(s.y,minY,pad);
  return s;
 }
 function viewInfo(){
 ...
 function paint(){
  ...
  Object.assign(viewport.dataset,{zoom:state.zoom.toFixed(4),panX:state.x.toFixed(2),panY:state.y.toFixed(2),scale:scale().toFixed(5),mode:state.mode,animating:String(Boolean(tween))});
 ...
 function focusElement(element,enabled=true,ratio=state.sectionRatio||1,anchor=null){
  if(!element)return;focused=element;state.sectionRatio=ratio;
 ...
 function zoomTo(value,point={x:vw/2,y:vh/2},enabled=true){
  const old=scale(),zoom=clamp(value,.02,16),ratio=fit*zoom/old;
 ...
 function snap(){
  ...
  focused=hit.element;focusElement(focused,true,1,hit.anchor);
 }
 function resize(){
 ...
 return{zoomTo,zoomBy,extents,reset:extents,move,focusElement,ensureComfortable,captureAnchor,holdAnchor,
