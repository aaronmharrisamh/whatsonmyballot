...
export function mountPaper(viewport,paper,saved,onChange,onView=()=>{}){
 ...
 function ensureComfortable(element,target){
  ...
  if(before.width*current>=viewport.clientWidth*.9)return;
  interrupt();
  ...
  state.mode='free';state.zoom=viewport.clientWidth/before.width/fit;
  state.x=-before.left*scale();state.y=point.y-sourceY*scale();paint(true);
 }
 function constrain(){
 ...
 function focusElement(element,animate=true){
  if(!element)return;focused=element;
  const {left,top,width}=position(element);
  state.mode='section';state.zoom=viewport.clientWidth/width/fit;
  state.x=-left*scale();state.y=-top*scale();paint(animate);
 }
 function resize(){
