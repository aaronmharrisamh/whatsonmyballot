...
function sourcePanel(page=null){
 return '<div class="source-panel"><strong>Original ballot layout reference</strong><p>Akron Township, Precinct 1AF · November 5, 2024'+(page?' · Page '+page:'')+'</p><p class="small">B.7 replaces the names with fictional entries. Its random guide marks are not on this PDF.</p><a href="'+h(pdfPath())+(page?'#page='+page:'')+'" target="_blank" rel="noopener noreferrer">'+icon('pdf')+' Open original sample PDF '+icon('source')+'</a></div>';
}
...
function noteTools(id,prefix,detailAction){
 const n=notesStore.get(id),open=opinionOpen(id),available=hasOpinion(id),exception=isExceptionOpinion(id);
 return '<div class="b-tools-slot"><div class="b-reveal-inner"><div class="b-row-tools">'+button(exception?exceptionScaleIcon:scaleIcon,'opinion','b-opinion-chip'+(!available?' is-empty':''),'data-id="'+h(id)+'" aria-label="'+(available?'Opinion on ':'No opinion added for ')+h(n.label)+'" aria-expanded="'+open+'" aria-controls="'+prefix+'-opinion-'+h(id)+'" '+(!available?'disabled title="No opinion added"':''))+button(searchIcon,detailAction,'b-icon-btn','data-id="'+h(id)+'" aria-label="More information about '+h(n.label)+'"')+'</div></div></div>';
}
...
function markUrl(r,chosen){return new URL('marks/slate-'+(chosen?'filled':'empty')+'.svg',import.meta.url).href;}

function markImage(r,id,chosen,kind,prefix='section'){
 return '<span class="b-oval-stack" data-oval="'+h(id)+'" data-personal="'+chosen+'"><img class="b-choice-mark" data-mark="'+h(id)+'" data-mark-kind="'+kind+'" data-race-id="'+h(r.id)+'" src="'+markUrl(r,chosen)+'" width="48" height="32" alt="" aria-hidden="true">'+guideSymbol(id,chosen)+'</span>';
}
...
function paintFocus(){
 for(const n of choiceNodes('.b-ballot-section')){
  const selected=n.dataset.raceId===selectedSectionId;n.classList.toggle('is-focused',selected);n.classList.add('is-expanded');
...
function syncMarks(){
 choiceNodes('[data-mark]').forEach(el=>{
  ...
  el.src=markUrl(r,chosen);const stack=el.closest('.b-oval-stack');if(stack){stack.dataset.personal=String(chosen);const hint=stack.querySelector('[data-guide-symbol]');if(hint)hint.hidden=chosen||!showGuideMarks||!hasGuideMark(id);}
 });
}
...
function selectSection(id,pan=true){
 const r=get('contests',id);if(!r)return;if(id!==selectedSectionId)revealedSectionId=null;selectedSectionId=id;model.visit(id);
 if(!ballotDialog.open){screen='race';render({focus:true});return;}
 const page=r.sourceRefs[0].viewerPage;
 if(page!==paperPage){paperPage=page;renderBallot({focusSection:pan});}
 else{paintFocus();updatePaperControls();const section=ballotDialog.querySelector('#paper-'+CSS.escape(id));paperController?.selected(section);if(pan)paperController?.panToElement(section,true);}
...
let onlyRecommendations=true,revealedSectionId=null;
...
function recommendationsToggle(prefix){
 return '<label class="b-guide-toggle"><input type="checkbox" role="switch" id="'+prefix+'-only-recommendations" data-only-recommendations '+(onlyRecommendations?'checked':'')+'><span class="b-switch-track" aria-hidden="true"></span><span>Only Recommendations</span></label>';
}
function sectionFilterControls(r,prefix){
 const marked=sectionChoiceIds(r).some(hasGuideMark);
 return '<p class="b-no-recommendations" data-no-recommendations="'+h(r.id)+'" '+(marked?'hidden':'')+'>There are no recommendations for this section. Use your best judgement!</p><div class="b-show-all" data-show-all-row="'+h(r.id)+'" '+(!onlyRecommendations?'hidden':'')+'><label><input type="checkbox" data-show-all="'+h(r.id)+'" '+(revealedSectionId===r.id?'checked':'')+'> <span>Show all choices</span></label></div>';
}
function syncRecommendationFilter(){
 choiceNodes('[data-only-recommendations]').forEach(el=>el.checked=onlyRecommendations);
 choiceNodes('.b-ballot-section').forEach(section=>{
  const id=section.dataset.raceId,r=get('contests',id),reveal=revealedSectionId===id;
  section.querySelectorAll('[data-guide-row]').forEach(row=>row.hidden=onlyRecommendations&&!reveal&&!hasGuideMark(row.dataset.guideRow));
  const box=section.querySelector('[data-show-all]');if(box)box.checked=reveal;
  const control=section.querySelector('[data-show-all-row]');if(control)control.hidden=!onlyRecommendations;
...
function changeRecommendations(value){
 ...
 onlyRecommendations=Boolean(value);revealedSectionId=null;saveReaderPreferences();syncRecommendationFilter();
...
const opinionOpen=id=>hasOpinion(id)&&(opinionOverrides.has(id)?opinionOverrides.get(id):showGuideMarks&&hasGuideMark(id));
...
function guideSymbol(id,chosen=false){
 ...
 return '<span class="b-guide-symbol" data-guide-symbol="'+h(id)+'" aria-hidden="true" '+(showGuideMarks&&a?.mark!=='none'&&!chosen?'':'hidden')+'>'+guideGlyph(a?.mark)+'</span>';
}
...
function opinionPanel(id,prefix){
 const open=opinionOpen(id),marked=showGuideMarks&&hasGuideMark(id);
 return '<div class="b-opinion-slot" data-opinion-slot="'+h(id)+'" data-open="'+open+'" '+(!open?'inert':'')+'><div class="b-reveal-inner"><aside class="b-inline-opinion'+(marked?' is-guide-opinion':'')+'" id="'+prefix+'-opinion-'+h(id)+'" data-opinion-panel="'+h(id)+'" '+annotationStyle(annotationStore.get(id))+' aria-label="Opinion on '+h(notesStore.get(id).label)+'">'+opinionBody(id)+'</aside></div></div>';
}
function syncOpinionPanels(id=null){
 choiceNodes('[data-opinion-slot]').forEach(slot=>{
  ...
  const panel=slot.querySelector('[data-opinion-panel]');panel.classList.toggle('is-guide-opinion',showGuideMarks&&hasGuideMark(key));panel.innerHTML=opinionBody(key);
 });
 choiceNodes('[data-action=opinion]').forEach(el=>{
  const key=el.dataset.id;if(id&&key!==id)return;
  const available=hasOpinion(key);el.disabled=!available;el.classList.toggle('is-empty',!available);
...
function paperView(){
 ...
 return '<div class="b-canvas-body"><div class="paper-viewport" id="paper-viewport" tabindex="0" role="region" aria-label="Fictional ballot viewer. Pinch to zoom. Arrow keys move the page."><div class="paper-document" id="paper-document" data-page="'+paperPage+'"><header class="b-paper-heading"><h2>Sample General Election</h2><p>Sample Township · Precinct DEMO</p><p class="b-paper-demo-note">Fictional sample · Page '+paperPage+' of 2</p></header><div class="paper-columns">'+[1,2,3].map(c=>'<div class="paper-column" data-column="'+c+'">'+content.contests.filter(r=>r.sourceRefs[0].viewerPage===paperPage&&r.sourceRefs[0].column===c).map(r=>ballotSection(r,true)).join('')+'</div>').join('')+'</div><footer class="b-paper-foot">Fictional names and random marks · Layout sample only</footer></div><div id="paper-focus-frame" class="b-focus-frame" aria-hidden="true" hidden></div>'+button('<svg class="icon b-return-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M4 12h15m-6-6 6 6-6 6"/></svg><span></span>','refocus-section','b-section-indicator','id="section-indicator" hidden')+'</div><div class="b-zoom-slider" role="group" aria-label="Ballot zoom controls">'+button('+','zoom-in','b-zoom-step','aria-label="Zoom in"')+'<input id="ballot-zoom" type="range" min="0" max="100" step=".1" value="50" aria-label="Ballot zoom">'+button('−','zoom-out','b-zoom-step','aria-label="Zoom out"')+'</div><div class="b-idle-hint" id="pinch-hint" aria-hidden="true">'+extentsIcon+'<span>Pinch &amp; Zoom</span></div></div><nav class="b-ballot-toolbar" aria-label="Ballot view and sections">'+button(extentsIcon+'<span>Zoom</span>','zoom-extents','','aria-label="Zoom into the selected section" title="Zoom into the selected section"')+'<label class="b-section-zoom btn"><input type="checkbox" id="section-zoom" '+(tightSection?'checked':'')+' aria-label="Snap to a section after dragging">'+sectionIcon+'<span>Snap</span></label>'+button(icon('back')+'<span>Prev</span>','section-prev','','aria-label="Previous ballot section" '+(i===0?'disabled':''))+button('<span>Next</span>'+icon('next'),'section-next','','aria-label="Next ballot section" '+(i===content.contests.length-1?'disabled':''))+'</nav><span class="sr-only" id="section-position">Section '+(i+1)+' of '+content.contests.length+'</span>';
}
...
function raceView(){
 const r=currentRace();
 return '<div class="b-section-view"><div class="b-section-intro"><div><h1 tabindex="-1">Voting Guide for <span>'+h(area().displayLabel)+'</span></h1><p class="b-demo-label">Fictional options · Sample opinion guide</p></div></div><div id="guide-progress">'+renderProgress(content.contests,selectedSectionId,id=>model.count(id),activeBallot.pages)+'</div><div class="b-guide-options">'+recommendationsToggle('readable')+'</div>'+adjacentChip(-1)+'<div class="b-readable-paper">'+ballotSection(r)+'</div>'+adjacentChip(1)+'<div class="b-adjacent b-back-start">'+button(icon('back')+'<span>Back to Start</span>','back-to-start','b-step-chip')+'</div><div class="selection-message" id="selection-feedback" role="status" hidden></div></div>';
}
...
function renderBallot({reset=false,focusSection=false}={}){
 ...
 if(focusSection)paperController.panToElement(selected,true);else if(reset)paperController.extents(false);
...
function helpView(){
 return '<div class="race-layout b-help"><h1 tabindex="-1">Explore the example.</h1><section class="help-card"><h2>Read the opinion guide</h2><p>A down arrow and RECOMMENDED label suggest a main choice. OR and OPTIONAL show another option. These hints leave the oval empty. A solid oval means you made a choice.</p><p>Only Recommendations starts on. Turn it off to see every choice, or use Show all choices for one section. That section setting resets when you move to another section. Hidden choices remain saved.</p><p>A scale with an asterisk opens an opinion about a choice the guide does not recommend. A faded, dashed button means no opinion was added.</p></section><section class="help-card"><h2>Explore the ballot</h2><p>View opens Ballot Viewer. Its recommendation switch also controls the readable view. Zoom switches between the whole page and the selected section. The wheel, pinch, slider, plus, and minus adjust zoom within those limits.</p><p>Next and Prev move to the section top. With Snap on, the wheel scrolls and a released drag recenters on a section. Navigation keeps the current zoom within its limits. Tap the backdrop or X to close the viewer.</p></section><section class="help-card"><h2>Choose an area or edit the demo</h2><p>District lets you select an area. Use this area activates it. Areas without data keep Menu and Admin available. Create ballot is a placeholder.</p><p>Menu contains Demo, Admin, and Help. Fresh Start clears personal choices and returns to the first section with Only Recommendations on. Saved text and guide marks stay in this browser.</p></section>'+(hasActiveBallot()?sourcePanel():'')+'<p class="small muted">App '+h(APP_VERSION_LABEL)+'</p></div>';
}
...
function render({focus=false,keepScroll=false}={}){
 ...
 document.body.dataset.design='B.7';document.body.dataset.screen=screen;document.body.dataset.areaAvailable=String(available);
...
function go(next,raceId=null){
 transition(()=>{closeMenu(false);finishBallot(false);screen=next==='home'?'race':next;
  ...
  revealedSectionId=null;if(raceId){selectedSectionId=raceId;model.visit(raceId);}render({focus:true});
 });
}
...
function updatePaperControls(){
 ...
 const zoomButton=ballotDialog.querySelector('[data-action=zoom-extents]');if(zoomButton){zoomButton.setAttribute('aria-pressed',String(paperState.zoomView!=='page'));const label=paperState.zoomView==='page'?'Zoom into the selected section':'Zoom out to the whole ballot page';zoomButton.setAttribute('aria-label',label);zoomButton.title=label;}
...
document.addEventListener('change', (event) => {
  ...
  if(el.matches('[data-show-all]')){const anchor=paperController?.captureAnchor(el.closest('.b-ballot-section'));revealedSectionId=el.checked?el.dataset.showAll:null;syncRecommendationFilter();paperController?.layoutChanged(anchor);}
  if(el.id==='section-zoom'){tightSection=el.checked;paperController?.setSnap(tightSection);}
...
async function init() {
 try{
  ...
  selectedSectionId=content.contests[0].id;variant={...settings.variants.find(e=>e.id==='A5'),id:'B.7',showSelectionExplanation:false};
  ...
  if(params.get('demo')==='fresh'){model.clearAll();onlyRecommendations=true;showGuideMarks=true;revealedSectionId=null;saveReaderPreferences();selectedSectionId=content.contests[0].id;screen='race';const url=new URL(location.href);url.searchParams.delete('demo');history.replaceState(null,'',url);}
...
Object.assign(actions, {
 'back-to-start':()=>{returnToReview=false;revealedSectionId=null;go('race',content.contests[0].id);},
...
function runDemo(action){
 ...
 confirmAction(action==='fresh'?'Fresh Start?':'Load example choices?',action==='fresh'?'Clear personal choices and return to the first section? The sample opinion guide and saved text stay as they are.':'Replace personal choices with the bundled sample choices?',action==='fresh'?'Start fresh':'Load example',()=>{
  finishBallot(false);if(action==='fresh'){model.clearAll();onlyRecommendations=true;showGuideMarks=true;revealedSectionId=null;saveReaderPreferences();selectedSectionId=content.contests[0].id;paperPage=1;screen='race';}else{model.example();screen='review';}render({focus:true});
 });
}
...
Object.assign(actions,{
 ...
 'create-ballot':()=>openDialog('Create ballot','<p>This feature is planned. No ballot is created in B.7.</p><p>Choose an area with ballot data to try the existing local editor.</p>',button('Close','close-dialog')),
 'demo-fresh':()=>runDemo('fresh'),'demo-example':()=>runDemo('example'),'demo-clear':()=>runDemo('clear'),'demo-restore':()=>runDemo('restore'),
 'refocus-section':()=>paperController?.panToElement(ballotDialog.querySelector('#paper-'+CSS.escape(selectedSectionId)),true),
 'zoom-extents':()=>paperController?.toggleZoom(ballotDialog.querySelector('#paper-'+CSS.escape(selectedSectionId))),
