...
function raceView(){
 const r=currentRace();
 return '<div class="b-section-view"><div class="b-section-intro"><div><span class="b-kicker">A CLOSER LOOK</span><h1 tabindex="-1">One section at a time.</h1><p>Section '+(sectionIndex()+1)+' of '+content.contests.length+' · 2024 sample</p></div>'+button(renderMinimap(content.contests,r.id),'race-list','b-minimap','aria-label="Ballot map: find a section" title="Find a section"')+'</div>'+viewSwitch()+'<div class="b-readable-paper">'+ballotSection(r)+'</div><div class="selection-message" id="selection-feedback" role="status">'+h(selectionText(r))+'</div><div class="button-row">'+button('Clear this section','clear-race','quiet')+'</div></div>';
}

function helpView(){
 return '<div class="race-layout b-help"><span class="b-kicker">A LITTLE GUIDANCE</span><h1 tabindex="-1">Keep your place.</h1><section class="help-card"><h2>Go from section to section</h2><p>Previous and Next move through all 28 printed sections. Tap a section to select it. The green border shows your place.</p><p>Zoom Section fits that section across the window. Drag down to read the rest. Zoom Extents shows the full page.</p></section><section class="help-card"><h2>Look around</h2><p>Pinch to zoom and drag to move. On a computer, scroll to move or hold Ctrl while scrolling to zoom. Arrow keys move the focused canvas. Home shows the full page.</p></section><section class="help-card"><h2>Read the notes</h2><p>The chevron opens one neutral sentence. The magnifying glass opens more information. The scales open an opinion with its author.</p><p>For a race with more than one choice, 1/2* means one of two possible choices is marked. You can choose fewer.</p><p>One section gives you the same content with large buttons and ordinary scrolling.</p></section><section class="help-card"><h2>About this sample</h2><p>This is the historical November 2024 Akron Township 1AF sample. Profiles are examples. Opinions use neutral text by a fictional sample editor.</p>'+sourcePanel()+'</section><p class="small muted">'+h(statusLabel())+' App '+h(APP_VERSION_LABEL)+'</p>'+button('Clear personal choices','clear-all')+'</div>';
}
function render({focus=false,keepScroll=false}={}){
 ...
 document.body.dataset.design='B.2';document.body.dataset.screen=screen;
 if(screen==='home'){
  paperController=mountPaper(document.querySelector('#paper-viewport'),document.querySelector('#paper-document'),paperState,state=>{paperState=state;updatePaperControls();},updateViewedSection);
...
function syncChoices(message='',error=false){
 ...
 const progress=root.querySelector('.b-progress-track i');if(progress)progress.style.width=model.answered()/28*100+'%';
 for(const input of root.querySelectorAll('[data-option]')){input.checked=(model.record(input.dataset.raceId).optionIds||[]).includes(input.dataset.option);input.closest('.candidate-card').classList.toggle('is-selected',input.checked);}
 syncMarks();
 ...
 root.querySelectorAll('[data-write-name]').forEach(el=>{const n=notesStore.get(el.dataset.writeName),name=model.record(n.raceId).writeIns[n.id]||'';el.textContent=name||'Write-in';});
...
function updatePaperControls(){
 const i=sectionIndex(),count=root.querySelector('#section-position');
 if(count)count.textContent='Section '+(i+1)+' of '+content.contests.length;
 const prev=root.querySelector('[data-action=section-prev]'),next=root.querySelector('[data-action=section-next]');
...
function updateViewedSection(id){
 const label=root.querySelector('#viewed-section'),race=id?get('contests',id):null;
...
const actions = {
  ...
  'clear-all': () => confirmAction('Clear personal choices?', 'This clears your choices for this sample ballot. Ballot text and Admin edits stay as they are.', 'Clear choices', () => { model.clearAll(); selectedSectionId=content.contests[0].id;paperPage=1;focusPending=true;screen = 'home'; render({ focus: true }); }),
...
document.addEventListener('click', async (event) => { const section=event.target.closest('.b-ballot-section');if(screen==='home'&&section&&!busy){const target=event.target.closest('button,label')||event.target;prepareInteraction(section,target);}
  ...
  if (event.data.type === 'demo' && ['fresh','example','clear','restore'].includes(event.data.action)) {
    ...
    if (action === 'restore') actions['restore-content']();
      ...
      : 'Replace personal choices with one labeled demo write-in? No real candidate is recommended.', action === 'fresh' ? 'Start fresh' : 'Load example', () => {
        if (action === 'fresh') { model.clearAll(); selectedSectionId=content.contests[0].id;paperPage=1;focusPending=true;screen = 'home'; } else { model.example(); screen = 'review'; reviewMode = 'rows'; }
        render({ focus: true });
      });
  }
...
async function init() {
  try {
    ...
    const params = new URLSearchParams(location.search); variant = { ...settings.variants.find(entry => entry.id === 'A5'), id: 'B.2', showSelectionExplanation: false };
