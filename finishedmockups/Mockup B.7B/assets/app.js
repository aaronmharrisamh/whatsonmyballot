...
function sourcePanel(page=null){
 return '<div class="source-panel"><strong>Original ballot layout reference</strong><p>Akron Township, Precinct 1AF · November 5, 2024'+(page?' · Page '+page:'')+'</p><p class="small">B.7B replaces the names with fictional entries. Its random guide marks are not on this PDF.</p><a href="'+h(pdfPath())+(page?'#page='+page:'')+'" target="_blank" rel="noopener noreferrer">'+icon('pdf')+' Open original sample PDF '+icon('source')+'</a></div>';
}
...
const noteLabels={synopsis:'Neutral synopsis (one sentence; optional)',author:'Opinion author',opinion:'Opinion text',sourceUrl:'Source link (HTTPS; optional)'};
...
function syncMarks(){
 choiceNodes('[data-mark]').forEach(el=>{
  ...
  el.src=markUrl(r,chosen);const stack=el.closest('.b-oval-stack');if(stack){stack.dataset.personal=String(chosen);const hint=stack.querySelector('[data-guide-symbol]');if(hint)hint.hidden=!showGuideMarks||!hasGuideMark(id);}
 });
}
...
function selectSection(id,pan=true){
 ...
 else{paintFocus();updatePaperControls();paperController?.setSnap(tightSection);const section=ballotDialog.querySelector('#paper-'+CSS.escape(id));if(pan)paperController?.focusElement(section,true,1);}
...
function raceView(){
 const r=currentRace();
 return '<div class="b-section-view"><div class="b-section-intro"><div><h1 tabindex="-1">Voting Guide for <span>'+h(area().displayLabel)+'</span></h1><p class="b-demo-label">Fictional options · Sample opinion guide</p></div></div><div id="guide-progress">'+renderProgress(content.contests,selectedSectionId,id=>model.count(id),activeBallot.pages)+'</div><div class="b-guide-options">'+recommendationsToggle('readable')+'</div>'+adjacentChip(-1)+'<div class="b-readable-paper">'+ballotSection(r)+'</div>'+adjacentChip(1)+'<div class="b-adjacent b-back-start" '+(ballotUI.showBackToStart?'':'hidden')+'>'+button(icon('back')+'<span>Back to Start</span>','back-to-start','b-step-chip')+'</div><div class="selection-message" id="selection-feedback" role="status" hidden></div></div>';
}
...
function render({focus=false,keepScroll=false}={}){
 ...
 document.body.dataset.design='B.7B';document.body.dataset.screen=screen;document.body.dataset.areaAvailable=String(available);
...
document.addEventListener('change', (event) => {
  ...
  if(el.id==='section-zoom')setSnapMode(el.checked);
...
async function init() {
 try{
  ...
  selectedSectionId=content.contests[0].id;variant={...settings.variants.find(e=>e.id==='A5'),id:'B.7B',showSelectionExplanation:false};
...
Object.assign(actions,{
 ...
 'create-ballot':()=>openDialog('Create ballot','<p>This feature is planned. No ballot is created in B.7B.</p><p>Choose an area with ballot data to try the existing local editor.</p>',button('Close','close-dialog')),
