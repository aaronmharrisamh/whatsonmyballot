...
function recordAt(value,path){let node=value,last=null;for(const part of path.split('/').slice(1)){if(node&&typeof node==='object'&&typeof node.id==='string')last=node.id;node=node?.[part.replaceAll('~1','/').replaceAll('~0','~')];}return node?.id||last;}
...
function shapeGuard(value){
 ...
 while(stack.length){const [v,depth]=stack.pop();if(v!==null&&(!['object','string','number','boolean'].includes(typeof v)||(typeof v==='number'&&!Number.isFinite(v))))fail('JSN-E03',{message:'Use JSON values without functions, undefined, or non-finite numbers.'});if(depth>80||++nodes>1000000)fail('JSN-E03',{message:'JSON nesting or complexity is too large.'});
...
export function validateStructure(value){
 ...
 if(!validateSchema(value)){
  const errors=validateSchema.errors.slice(0,12).map(e=>{
   ...
   return diagnostic(code,{path:p,message:'Invalid field: '+e.message,recordId:recordAt(value,p),receivedVersion:value.schemaVersion});
...
export async function validateCollection(value,options={}){
 try{options.signal?.throwIfAborted();validateStructure(value);let json;try{json=JSON.stringify(value);}catch{fail('JSN-E03');}
 const fileBytes=Math.max(options.fileBytes||0,encode.encode(json).length);if(fileBytes>LIMITS.fileBytes)fail('JSN-E09',{actualBytes:fileBytes,limitBytes:LIMITS.fileBytes});
 ...
 }catch(error){if(error instanceof CollectionError)for(const d of error.diagnostics){d.receivedVersion??=typeof value?.schemaVersion==='string'?value.schemaVersion:null;d.recordId??=recordAt(value,d.path);}throw error;}
}
export async function parseCollection(input,{fileName=input?.name,signal,...options}={}){
 ...
 let bytes;if(typeof input==='string')bytes=encode.encode(input);else if(input instanceof Blob){if(input.size>LIMITS.fileBytes)fail('JSN-E09',{actualBytes:input.size,limitBytes:LIMITS.fileBytes});try{bytes=await readBounded(input.stream(),LIMITS.fileBytes,{signal});}catch(error){if(error instanceof CollectionError||error.name==='AbortError')throw error;fail('JSN-E01',{message:'The JSON file could not be read.'});}}else if(input instanceof Uint8Array)bytes=input;else fail('JSN-E01');
