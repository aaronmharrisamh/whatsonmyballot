...
import { clone, canonical, applyFields, collections, editableValues } from './content-fields.js';
...
export function createContentStore({ seed, original, storage, basePath, locks = null }) {
  ...
  async function commit(content, expectedRevision, { replaceUnreadable = false } = {}) {
    const operation = () => {
      ...
      const serialized = JSON.stringify({ format: 'whatsonmyballot-local-content', schemaVersion: '1.0.0', current: next, previous: current });
