...
function pdfRace(race,size,showCategory=true){
 const stack=[
  
  {text:race.title,fontSize:size,bold:true,lineHeight:1.02,margin:[0,0,0,2]},
  ...(race.instruction?[{text:race.instruction,fontSize:10.5,color:'#435767',margin:[0,0,0,2]}]:[]),
 ...
 return {table:{widths:['*'],dontBreakRows:true,body:[[{stack}]]},layout:{
  hLineWidth:()=>.6,vLineWidth:()=>.6,hLineColor:()=>'#becbd4',vLineColor:()=>'#becbd4',
...
function definition(state,sides,size,pageSize,onPageCount){
 return {
  ...
  info:{title:'Ballot with Recommendations',subject:state.areaLabel,creator:'What’s on my ballot · B.7F'},
  ...
  content:sides.map((side,index)=>({
   ...
   stack:[
    ...
    {text:'Empty ovals are for practice. Arrows show recommendations; OR shows an option.',fontSize:10.5,color:'#435767',margin:[0,0,0,8]},
...
function printChoice(choice){
 const color=colors(choice.color);
 return '<div class="b-print-choice" data-print-choice="'+esc(choice.id)+'" style="--print-ink:'+color.ink+';--print-tint:'+color.tint+'"><div class="b-print-choice-top"><span class="b-print-kind">'+(choice.mark==='this'?'↓ Recommended':'OR · Optional')+'</span> <strong>'+esc(choice.label)+'</strong></div>'+
...
export function prepareGuidePrint(options){
 ...
 root.innerHTML='<header class="b-print-heading"><h1>My Guide</h1><p>'+esc(state.areaLabel)+'</p></header><div class="b-print-columns">'+
 (state.groups.length?state.groups.map(group=>'<section class="b-print-group"><h2>'+esc(group.label)+'</h2>'+group.sections.map(section=>section.contests.map(race=>'<section class="b-print-race" data-print-race="'+esc(race.id)+'"><div class="b-print-leading"><h3>'+esc(contextualTitle(race.title,section.title))+'</h3>'+printChoice(race.choices[0])+'</div>'+race.choices.slice(1).map(printChoice).join('')+'</section>').join('')).join('')+'</section>').join(''):'<p>No recommendations have been added to this guide.</p>')+'</div>';
