...
export function mountPaper(viewport,paper,saved,onChange,onView=()=>{},onSnap=()=>{}){
 ...
 function recoverSectionEnd(element){
  if(!element?.isConnected)return false;
  const p=position(element),bottom=state.y+(p.top+p.height)*scale();
  // Do not pin a reader who is still moving through the section. Recover only
  // after its end has passed the middle of the view into empty paper space.
  if(bottom>=vh*.5)return false;
  focused=element;const zoom=state.zoom;
  animate(()=>{
   const box=position(element),sc=fit*zoom,height=box.height*sc;
   // Tall sections show their last half-window plus Next. Short sections remain
   // fully visible. Keep this zoom: recovery is a pan, not another zoom gesture.
 ...
 function snap(){
  ...
  if(!hit){recoverSectionEnd(focused);return;}
  // A neighboring section can still take focus when it is near the center.
  // Only empty space below the nearest section triggers end recovery.
  const box=position(hit.element),bottom=state.y+(box.top+box.height)*scale();
  onSnap(hit.element.dataset.raceId);
  focused=hit.element;
  if(bottom<vh*.5&&recoverSectionEnd(focused))return;
