import { adaptBallot, exportBallot } from './ballot-adapter.js';
import { isSafeText, validateBallot } from './ballot-contract.js';
// B-series notes are a versioned companion to the original ballot content.
// B.9A adds an independent header-opinion highlight; it is not a vote recommendation.
...
export function createNotesStore(original,storage,basePath,ballot=null){
 ...
 function validate(next){
  ...
  for(const r of next.records){
   ...
   for(const k of fields)if(typeof r[k]!=='string'||r[k].length>({synopsis:20000,author:1000,opinion:50000,sourceUrl:4000}[k])||!isSafeText(r[k]))throw Error('A note field is missing or too long.');
   validateHeader(r);
   if(r.author&&!r.author.trim())throw Error('Use a nonempty author name, or clear the author field.');
   if(r.opinion.trim()&&!r.author.trim())throw Error('Add the opinion author before saving an opinion.');
   if(r.sourceUrl){if(r.sourceUrl.startsWith('attachment:')){if(!ballot?.attachments?.some(item=>'attachment:'+item.id===r.sourceUrl))throw Error('Use an attachment ID from the active ballot.');}else{let url;try{url=new URL(r.sourceUrl);}catch{throw Error('Use a full HTTPS source link.');}if(url.protocol!=='https:'||url.username||url.password)throw Error('Use an HTTPS source link without sign-in details.');}}
  }
  if(ballot){const base=adaptBallot(ballot,'notes-validation');const result=validateBallot(exportBallot(ballot,{content:base.original,notes:next}));if(!result.ok)throw Error(result.errors.join('\n'));}
  return next;
 }
 validate(original);
 function load(){
 ...
 function validDraft(r){
  ...
  const base=ids.get(r.id);
  if(!exactKeys(r.values,editableFields(base))||fields.some(k=>typeof r.values[k]!=='string'||r.values[k].length>({synopsis:20000,author:1000,opinion:50000,sourceUrl:4000}[k])))return false;
