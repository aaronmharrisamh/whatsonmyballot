import { parseBallot, validateBallot, MAX_IMPORT_BYTES as MAX_BALLOT_BYTES } from './ballot-contract.js';
import { prepareResources } from './ballot-resources.js';
import { adaptBallot, exportBallot, setAttachmentResolver, resolveContentUrl, getNoteSources, getDetailSources, getBallotSources } from './ballot-adapter.js';
import { openWorkspace } from './ballot-workspace.js';
...
import { makeBallotPdf, printGuide, prepareGuidePrint } from './outputs.js';
...
let outputBusy=false,outputMessage='';
let workspace,activeBundle,activeResources=null,importBusy=false,importMessage='';
let appInitialized=false,pendingBallotImport=null,importDrainTimer=null,importErrorMessage='';
...
const pdfPath = () => resolveContentUrl(content.officialPdfs[0]?.path||'');
...
function sourcePanel(){
 const links=getBallotSources(activeBundle,content);
 return links.length?'<div class="source-panel"><strong>Ballot sources</strong>'+links.map(source=>'<p><a href="'+h(source.url)+'" target="_blank" rel="noopener noreferrer">'+h(source.title)+' '+icon('source')+'</a></p>').join('')+'</div>':'';
}


function linkList(links,label='Source'){
 return links.filter(item=>item.url).map(item=>'<p><a href="'+h(item.url)+'" target="_blank" rel="noopener noreferrer">'+h(item.title||label)+' '+icon('source')+'</a></p>').join('');
}
function detailBody(d,ownerId=''){
 const paragraphs=d.paragraphs.length?d.paragraphs:d.summary;
 const sources=getDetailSources(activeBundle,ownerId,content);
 return (d.synopsis?'<p>'+h(d.synopsis)+'</p>':'')+(d.overview?'<p>'+h(d.overview)+'</p>':'')+
 '<div class="detail-copy">'+paragraphs.map(p=>'<p>'+h(p)+'</p>').join('')+'</div>'+
 linkList(sources)+(d.websiteUrl?'<p><a class="btn compact" href="'+h(d.websiteUrl)+'" target="_blank" rel="noopener noreferrer">Candidate website '+icon('source')+'</a></p>':'')+
 d.documents.map(doc=>button(h(doc.title||'Read details')+' '+icon('next'),'read-document','compact','data-id="'+h(doc.id)+'" '+(get('candidates',ownerId)?'data-candidate="'+h(ownerId)+'"':''))).join('');
}
...
function writeInValue(raceId,id){const values=model.record(raceId).writeIns;return Object.hasOwn(values,id)?values[id]:'';}
...
const noteLabels={synopsis:'Summary (optional)',author:'Opinion author',opinion:'Opinion text',sourceUrl:'Source link (HTTPS or attachment reference; optional)',headerHighlighted:'Highlight this header opinion',headerColor:'Header opinion color'};
...
function writeInRow(w,r,prefix){
 const name=writeInValue(r.id,w.id),a=annotationStore.get(w.id);
 return '<div class="b-write-row '+annotationClass(w.id)+'" data-write-row="'+h(w.id)+'" data-guide-row="'+h(w.id)+'" '+annotationStyle(a)+'><div class="b-candidate-row">'+button(markImage(r,w.id,Boolean(name.trim()),'write-in',prefix),'write-in','b-tick b-write-tick','data-id="'+h(w.id)+'" data-race-id="'+h(r.id)+'" aria-label="Edit write-in for '+h(r.officialTitle)+'"')+inlineText(w.id,prefix,'<span class="candidate-name" data-write-name="'+h(w.id)+'">'+h(name||w.officialLabel||w.displayLabel||'Write-in')+'</span>')+noteTools(w.id,prefix,'note-info')+'</div>'+opinionPanel(w.id,prefix)+annotationPill(w.id)+'</div>';
}

function showWriteIn(el){
 const r=get('contests',el.dataset.raceId),id=el.dataset.id;
 openDialog('Write-in / '+r.officialTitle,'<p>A name counts toward the choice limit for this race.</p><label class="editor-field">Write-in name<input type="text" maxlength="120" data-write-in="'+h(id)+'" data-race-id="'+h(r.id)+'" value="'+h(writeInValue(r.id,id))+'" autocomplete="off"></label><p class="small">Your guide saves this name as you type.</p><p id="write-status" role="status"></p>',button('Done','close-dialog'));
}
function paintFocus(){
...
function syncMarks(){
 choiceNodes('[data-mark]').forEach(el=>{
  ...
  const chosen=el.dataset.markKind==='party'?(choice.optionIds||[]).includes(id):el.dataset.markKind==='write-in'?Boolean(writeInValue(r.id,id).trim()):choice.candidateIds.includes(id);
...
function showNoteInfo(id){
 const option=activeBundle.contests.flatMap(r=>r.options).find(o=>o.id===id),photo=option?.photoAttachmentId?activeResources.url(option.photoAttachmentId):'';
 const n=notesStore.get(id),d=detailContent(content,n,id);if(!d.available)return;
 const r=get('contests',n.raceId);
 openDialog(n.label,(photo?'<div class="portrait-line"><img src="'+h(photo)+'" alt="'+h(option.label)+'"></div>':'')+detailBody(d,id)+notePill(id)+sourcePanel(r.sourceRefs[0].viewerPage));
}
function notesAdminPanel(){
 return '<section class="help-card"><h2>Opinions and details</h2><p>Opinions and neutral details are optional. Add an author when you add an opinion.</p>'+(notesStore.warning?'<p class="notice">'+h(notesStore.warning)+'</p>':'')+'<label class="editor-field">Note<select id="note-select">'+notesStore.value.records.map(n=>'<option value="'+h(n.id)+'">'+h(n.kind+' / '+n.label)+'</option>').join('')+'</select></label>'+button(scaleIcon+' Edit selected note','edit-selected-note')+button('Restore loaded notes','restore-notes','quiet')+'</section>';
}
function adminView(){return originalAdminView();}
function noteEditorFields(values){
 return Object.entries(values).map(([k,v])=>{
  ...
  return '<label class="editor-field">'+h(noteLabels[k])+'<textarea data-edit-field="'+k+'" rows="'+(k==='opinion'?5:2)+'" maxlength="'+(k==='opinion'?50000:k==='sourceUrl'?4000:k==='author'?1000:20000)+'">'+h(v)+'</textarea></label>';
 }).join('');
}
...
const sectionHasRecommendations=r=>sectionChoiceIds(r).some(hasGuideMark);
const ballotHasRecommendations=()=>Boolean(content?.contests.some(sectionHasRecommendations));
function recommendationsToggle(prefix){
 return '<label class="b-guide-toggle"><input type="checkbox" role="switch" id="'+prefix+'-only-recommendations" data-only-recommendations '+(onlyRecommendations?'checked':'')+' '+(ballotHasRecommendations()?'':'disabled')+'><span class="b-switch-track" aria-hidden="true"></span><span data-filter-label>'+(onlyRecommendations?'Only Recommended':'Showing All')+'</span></label>';
}
function sectionFilterControls(r,prefix){
 const marked=sectionChoiceIds(r).some(hasGuideMark);
 return '<p class="b-no-recommendations" data-no-recommendations="'+h(r.id)+'" '+(marked?'hidden':'')+'>No recommendations for this section. All choices are shown.</p>';
}
function syncRecommendationFilter(){
 const hasRecommendations=ballotHasRecommendations();
 if(!hasRecommendations&&onlyRecommendations){onlyRecommendations=false;saveReaderPreferences();}
 choiceNodes('[data-only-recommendations]').forEach(el=>{el.checked=onlyRecommendations;el.disabled=!hasRecommendations;el.closest('label').querySelector('[data-filter-label]').textContent=onlyRecommendations?'Only Recommended':'Showing All';});
 choiceNodes('.b-ballot-section').forEach(section=>{
  const r=get('contests',section.dataset.raceId),filterSection=onlyRecommendations&&sectionHasRecommendations(r);
  section.querySelectorAll('[data-guide-row]').forEach(row=>row.hidden=filterSection&&!hasGuideMark(row.dataset.guideRow));
...
function changeRecommendations(value){
 ...
 onlyRecommendations=Boolean(value)&&ballotHasRecommendations();saveReaderPreferences();syncRecommendationFilter();
 paperController?.layoutChanged(anchor);
 announce(onlyRecommendations?'Recommendations are shown. Sections without recommendations show all choices.':'All choices are shown.');
}
...
function annotationsAdminPanel(){
 return '<section class="help-card"><h2>Guide marks</h2><p>THIS is a main choice. OR is another option. These marks do not fill a reader’s ovals.</p>'+(annotationStore.warning?'<p class="notice">'+h(annotationStore.warning)+'</p>':'')+'<label class="editor-field">Ballot option<select id="annotation-select">'+annotationStore.value.records.map(a=>'<option value="'+h(a.id)+'">'+h(a.kind+' / '+a.label)+'</option>').join('')+'</select></label>'+button(icon('edit')+' Edit guide mark','edit-selected-annotation')+button('Restore loaded guide marks','restore-annotations','quiet')+'</section>';
}
...
function homeView(){return raceView();}
function columnRaces(page,column){return resolveBallotLayout(content).columns.find(c=>c.viewerPage===page&&c.column===column)?.contests||[];}
function columnAdjacent(index,delta){
 const columns=resolveBallotLayout(content).columns,forward=delta>0;
 let target=index+delta;while(target>=0&&target<columns.length&&!columns[target].contests.length)target+=delta;
 const destination=forward?columns[target]?.contests[0]:columns[target]?.contests.at(-1);
 if(!destination&&!forward)return '';
 const number=destination?content.contests.findIndex(r=>r.id===destination.id)+1:0;
 const label=destination?(forward?'Next: ':'Back: ')+headingText(destination)+' ('+number+'/'+content.contests.length+')':'Next: My Guide';
 return '<div class="b-column-adjacent '+(forward?'b-column-adjacent-next':'b-column-adjacent-back')+'">'+button((forward?'':icon('back'))+'<span>'+h(label)+'</span>'+(forward?icon('next'):''),destination?'locate-race':'nav-review','b-step-chip','data-section-nav="true" '+(forward?'data-column-next="true" data-snap-next ':'')+(destination?'data-id="'+h(destination.id)+'" ':'')+'title="'+h(label)+'"')+'</div>';
}
function paperColumn(column){
 const races=column.contests;
 return '<div class="paper-column" data-column="'+column.column+'" data-column-index="'+column.index+'">'+(races.length?columnAdjacent(column.index,-1)+races.map(r=>ballotSection(r,true)).join('')+columnAdjacent(column.index,1):'')+'</div>';
}
function paperSheets(){
 return resolveBallotLayout(content).pages.map(page=>'<div class="b-paper-side" data-page="'+page.viewerPage+'" style="--page-width:'+page.width+'px;--page-columns:'+page.columns.length+'"><h2 class="b-paper-side-label" title="'+h(page.label)+'">'+h(page.label)+'</h2><article class="b-ballot-sheet" aria-label="'+h(page.label)+'"><header class="b-paper-heading"><h2>'+h(activeBundle.election.title)+'</h2><p>'+h(area().displayLabel)+'</p><p class="b-paper-demo-note">'+(activeBundle.election.isDemo?'Fictional sample · ':'')+h(page.label)+'</p></header><div class="paper-columns">'+page.columns.map(paperColumn).join('')+'</div><footer class="b-paper-foot">'+h(activeBundle.title)+'</footer></article></div>').join('');
}
function paperView(){
 const i=sectionIndex();
 return '<div class="b-canvas-body"><div class="paper-viewport" id="paper-viewport" tabindex="0" role="region" aria-label="Ballot viewer. Pinch to zoom. Arrow keys move the page."><div class="b-paper-spread" id="paper-document">'+paperSheets()+'</div><div id="paper-focus-frame" class="b-focus-frame" aria-hidden="true" hidden></div>'+button('<svg class="icon b-return-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M4 12h15m-6-6 6 6-6 6"/></svg><span></span>','refocus-section','b-section-indicator','id="section-indicator" hidden')+'</div><div class="b-zoom-slider" role="group" aria-label="Ballot zoom controls">'+button('+','zoom-in','b-zoom-step','aria-label="Zoom in"')+'<input id="ballot-zoom" type="range" min="0" max="100" step=".1" value="50" aria-label="Ballot zoom">'+button('−','zoom-out','b-zoom-step','aria-label="Zoom out"')+'</div>'+button(extentsIcon,'view-all','b-all-button','aria-label="All — show every ballot page" title="All"')+'<div class="b-idle-hint" id="pinch-hint" aria-hidden="true">'+extentsIcon+'<span>Pinch &amp; Zoom</span></div></div><nav class="b-ballot-toolbar" aria-label="Ballot view and sections">'+'<label class="b-section-zoom btn"><input type="checkbox" id="section-zoom" '+(tightSection?'checked':'')+' aria-label="Snap to the section nearest the center">'+sectionIcon+'<span>Snap</span></label>'+button(icon('back')+'<span>Prev</span>','section-prev','','aria-label="Previous ballot section" '+(i===0?'disabled':''))+button('<span>Next</span>'+icon('next'),'section-next','','aria-label="Next ballot section" '+(i===content.contests.length-1?'disabled':''))+'</nav><span class="sr-only" id="section-position">Section '+(i+1)+' of '+content.contests.length+'</span>';
}
function sectionBand(race,paper){
 const section=get('sections',race.sectionId);
 if(paper){const ref=race.sourceRefs[0],column=columnRaces(ref.viewerPage,ref.column),index=column.findIndex(r=>r.id===race.id);if(column[index-1]?.sectionId===race.sectionId)return '';}
 return '<div class="b-section-band">'+h(section.officialTitle)+'</div>';
}

function optionRows(race,prefix,paper){
 const order=race.optionOrder||[...race.candidateIds.map(id=>({id,kind:'candidate'})),...race.optionIds.map(id=>({id,kind:'choice'})),...race.writeIns.map(w=>({id:w.id,kind:'write-in'}))];
 return order.map(item=>item.kind==='candidate'?candidateCard(get('candidates',item.id),race,paper):item.kind==='write-in'?writeInRow(race.writeIns.find(w=>w.id===item.id),race,prefix):partyCard(get('options',item.id),race,prefix)).join('');
}
function ballotSection(race,paper=false){
 const prefix=paper?'paper':'section',section=get('sections',race.sectionId);
 return '<section class="b-ballot-section '+(paper?'is-paper':'is-readable')+(race.id===selectedSectionId?' is-focused':'')+'" data-race-id="'+h(race.id)+'" '+(paper&&race===content.contests.at(-1)?'data-final-section="true" ':'')+'id="'+prefix+'-'+h(race.id)+'" aria-labelledby="'+prefix+'-title-'+h(race.id)+'">'+sectionBand(race,paper)+'<header class="b-race-heading"><div><h2 class="race-title" id="'+prefix+'-title-'+h(race.id)+'">'+race.officialTitleLines.map(h).join('<br>')+'</h2><p class="b-limit">'+h(race.officialSelectionInstruction)+'</p></div>'+noteTools(race.id,prefix,'race-detail')+opinionPanel(race.id,prefix)+'</header>'+editPill(race.id,'contest')+(race.kind==='straight-party'?'<p class="b-reference-note">This guide keeps your party mark separate. It does not fill other races.</p>':'')+'<fieldset class="b-choices" aria-labelledby="'+prefix+'-title-'+h(race.id)+'"><legend class="sr-only">Choices for '+h(race.officialTitle)+'</legend>'+optionRows(race,prefix,paper)+'</fieldset>'+sectionFilterControls(race,prefix)+'<div class="b-section-footer"><span class="b-race-status" data-choice-status data-race-id="'+h(race.id)+'">'+h(selectionText(race))+'</span>'+button('Clear Selection','clear-race','b-clear-chip','data-id="'+h(race.id)+'" aria-label="Clear choices in '+h(race.officialTitle)+'"')+'</div>'+'</section>';
}
function partyCard(o,r,prefix){
 const chosen=(model.record(r.id).optionIds||[]).includes(o.id),a=annotationStore.get(o.id);
 return '<div class="candidate-card b-candidate '+(chosen?'is-selected ':'')+annotationClass(o.id)+'" data-guide-row="'+h(o.id)+'" data-race-id="'+h(r.id)+'" '+annotationStyle(a)+'><div class="b-candidate-row">'+choiceTick(r,o.id,chosen,prefix,o.officialLabel,'party')+inlineText(o.id,prefix,'<span class="candidate-name">'+h(o.officialLabel)+'</span>'+(o.partyLabel?'<span class="candidate-party">'+h(o.partyLabel)+'</span>':''))+noteTools(o.id,prefix,'note-info')+'</div>'+opinionPanel(o.id,prefix)+annotationPill(o.id)+'</div>';
}
...
function writeInFields(race){
 return '<details class="write-in" '+(Object.values(model.record(race.id).writeIns).some(Boolean)?'open':'')+'><summary>Add a write-in</summary><div class="write-in-fields"><p class="small muted">A name counts toward the selection limit.</p>'+race.writeIns.map((slot,i)=>'<label for="'+h(slot.id)+'">Write-in '+(race.writeIns.length>1?i+1:'name')+'<input id="'+h(slot.id)+'" type="text" maxlength="120" data-write-in="'+h(slot.id)+'" data-race-id="'+h(race.id)+'" value="'+h(writeInValue(race.id,slot.id))+'" autocomplete="off"></label>').join('')+'</div></details>';
}
...
function raceView(){
 const r=currentRace();
 return '<div class="b-section-view"><div class="b-section-intro"><div><h1 tabindex="-1">Voting Guide for <span>'+h(area().displayLabel)+'</span></h1><p class="b-demo-label">'+(activeBundle.election.isDemo?'Fictional sample · ':'')+'Opinion guide</p></div></div><div id="guide-progress">'+renderProgress(content,selectedSectionId,id=>model.count(id))+'</div><div class="b-guide-options">'+recommendationsToggle('readable')+'</div>'+/* Upper Back chip hidden in B.9A; candidate for complete removal later: adjacentChip(-1). */'<div class="b-readable-paper">'+ballotSection(r)+'</div>'+adjacentChip(1)+'<div class="b-adjacent b-back-start" '+(ballotUI.showBackToStart?'':'hidden')+'>'+button(icon('back')+'<span>Back to Start</span>','back-to-start','b-step-chip')+'</div><div class="selection-message" id="selection-feedback" role="status" hidden></div></div>';
}

function dockMap(){
 return button(renderMinimap(content,selectedSectionId)+'<span>Ballot Viewer</span>','open-ballot','b-minimap b-dock-map','aria-label="Open Ballot Viewer. Section '+(sectionIndex()+1)+' of '+content.contests.length+'" aria-haspopup="dialog" aria-expanded="'+ballotDialog.open+'" aria-controls="ballot-dialog"');
}
function updateDockMap(){
 const map=root.querySelector('.b-dock-map');
 if(map){map.innerHTML=renderMinimap(content,selectedSectionId)+'<span>Ballot Viewer</span>';map.setAttribute('aria-label','Open Ballot Viewer. Section '+(sectionIndex()+1)+' of '+content.contests.length);map.setAttribute('aria-expanded',String(ballotDialog.open));}
 syncProgress();
}
function syncProgress(){
 const progress=root.querySelector('#guide-progress');if(progress)progress.innerHTML=renderProgress(content,selectedSectionId,id=>model.count(id));
}
...
function helpView(){
 return '<div class="race-layout b-help"><h1 tabindex="-1">Explore the ballot.</h1><section class="help-card"><h2>Read the opinion guide</h2><p>A down arrow and RECOMMENDED label suggest a main choice. OR and OPTIONAL show another option. These hints leave the oval empty. An outlined oval with a smaller filled oval means you made a choice. The recommendation arrow or OR stays visible and stops moving.</p><p>Ballots with recommendations start with Only Recommended. Sections without recommendations always show all choices. A ballot with no recommendations starts with Showing All, and the switch stays off. Hidden choices remain saved.</p><p>A scale with an asterisk opens an opinion about a choice the guide does not recommend. A faded, dashed button means no opinion was added.</p></section><section class="help-card"><h2>My Guide and practice choices</h2><p>My Guide shows the admin recommendations and their opinions. Your marks in Ballot are for practice and do not change this cheatsheet. On phones, use the section chips to jump through the guide.</p></section><section class="help-card"><h2>Explore the ballot</h2><p>View opens Ballot Viewer. Its recommendation switch also controls the readable view. All shows every ballot page together and turns Snap off. Turning Snap on finds the section nearest the center and fits it to the viewer width. The wheel, pinch, slider, plus, and minus adjust zoom within those limits.</p><p>Next, Prev, and section taps turn Snap on and fit the section. With Snap on, the wheel scrolls and a released drag recenters on a section. Pinch still changes the zoom. If you move beyond a column, the view returns to its nearest end. Back and Next at the ends of each column move to the adjoining column. Turning Snap off keeps your current view. Each new opening starts zoomed out with Snap off. Tap the backdrop or X to close the viewer.</p></section><section class="help-card"><h2>Choose an area or edit the demo</h2><p>District shows the location supplied by the loaded ballot. Drop one ballot JSON file anywhere in the app or preview to load it. You can also choose the file in Admin. A valid file replaces the current ballot immediately.</p><p>Menu contains Demo, Admin, and Help. Fresh Start clears personal choices and returns to the first section. Only Recommended starts on when the ballot has recommendations; otherwise Showing All stays on. Saved text and guide marks stay in this browser.</p></section>'+(hasActiveBallot()?sourcePanel():'')+'<p class="small muted">App '+h(APP_VERSION_LABEL)+'</p></div>';
}

function pageContext(){
 const pages={district:['District',districtIcon],race:['Ballot',icon('ballot')],review:['My Guide',icon('review')],admin:['Admin',icon('edit')],help:['Help',icon('help')]};
 const [label,glyph]=pages[screen]||pages.race;
 return '<div class="b-page-context"><span class="b-page-context-inner">'+glyph+'<span data-page-label>'+h(label)+'</span></span></div>';
}
function render({focus=false,keepScroll=false}={}){
 ...
 root.innerHTML='<div class="app-layout has-page-context"><header class="app-header b-masthead"><a class="b-brand" href="#" data-action="nav-district"><img src="assets/flag-mark.svg" alt="" width="38" height="38"><span>What’s on my ballot?<small>'+h(activeBundle.election.isDemo?'FICTIONAL BALLOT · UI DEMO':activeBundle.election.title)+'</small></span></a>'+button(menuIcon,'open-menu','b-help-button','id="app-menu-button" aria-label="Open menu" aria-haspopup="dialog" aria-controls="menu-dialog" aria-expanded="'+menuDialog.open+'"')+(admin&&available?'<div class="admin-ribbon"><span>'+icon('edit')+'Edit buttons on</span><button class="admin-exit" data-action="leave-admin">Exit Admin</button></div>':'')+'</header>'+pageContext()+'<main id="main" class="main-scroll" tabindex="-1"><div class="main-inner">'+(screen==='district'?districtView():screen==='race'?raceView():screen==='review'?reviewView():screen==='admin'?adminView():helpView())+'</div></main>'+(screen==='race'?'<nav class="flow-actions" aria-label="Section navigation"><div class="flow-action-inner">'+dockMap()+button(icon('back')+'<span>Back</span>','back','back-btn',sectionIndex()===0&&!returnToReview?'disabled':'')+button((returnToReview||sectionIndex()===content.contests.length-1?'Review':'Next')+icon('next'),'next','primary next-btn')+'</div></nav>':'<div class="b-no-flow"></div>')+'<nav class="bottom-nav" aria-label="Main navigation">'+[['district',districtIcon,'District'],['ballot',icon('ballot'),'Ballot'],['review',icon('review'),'My guide'],['view',tallBallotIcon,'View']].map(([action,glyph,label])=>'<button data-action="nav-'+action+'" '+(navActive===action?'aria-current="page"':'')+' '+(!available&&action!=='district'?'disabled':'')+'>'+glyph+label+'</button>').join('')+'</nav></div>';
 document.body.dataset.design='B.9A';document.body.dataset.screen=screen;document.body.dataset.areaAvailable=String(available);
 ...
 if(screen==='review'){
  const main=root.querySelector('.main-scroll'),nav=root.querySelector('.b-guide-jumps');
  if(nav){main.before(nav);root.querySelector('.app-layout').classList.add('has-guide-nav');}
  guideNavigation=mountGuideNav(main,root.querySelector('.b-guide-review'),nav);
  syncOutputControls();
 }
 presentImportError();
}
...
function syncChoices(message='',error=false){
 ...
 for(const input of document.querySelectorAll('[data-write-in]')){
  const value=writeInValue(input.dataset.raceId,input.dataset.writeIn);if(input.value!==value)input.value=value;
 }
 ...
 syncMarks();syncProgress();syncOpinionPanels();
 ...
 choiceNodes('[data-write-name]').forEach(el=>{const n=notesStore.get(el.dataset.writeName),name=writeInValue(n.raceId,n.id);el.textContent=name||n.label||'Write-in';});
...
function cheatsheetChoice(choice){
 return '<div class="b-cheatsheet-choice" data-guide-choice="'+h(choice.id)+'" data-guide-mark="'+choice.mark+'" '+annotationStyle(choice)+'><div class="b-cheatsheet-kind">'+guideChoiceSymbol(choice.mark)+'<span>'+(choice.mark==='this'?'Recommended':'Optional')+'</span></div><strong class="b-cheatsheet-name">'+h(choice.label)+'</strong>'+(choice.opinion.trim()?'<aside class="b-cheatsheet-opinion" aria-label="Opinion on '+h(choice.label)+'"><div class="b-cheatsheet-author">'+scaleIcon+'<span>Opinion'+(choice.author?' · '+h(choice.author):'')+'</span></div>'+choice.opinion.trim().split(/\n{2,}/).map(p=>'<p>'+h(p)+'</p>').join('')+linkList(getNoteSources(activeBundle,choice.id,notesStore.get(choice.id),content))+'</aside>':'')+'</div>';
}
function reviewSection(section,category,label=''){
 const compact=value=>value.toLowerCase().replace(/[\s-]/g,'');
 const redundant=compact(section.title)===compact(label)||compact(section.title)===compact(category+'section');
...
function rowReview(groups){
 const capsule=group=>'<section class="b-review-capsule" id="cheat-group-'+h(group.category)+'" data-review-category="'+h(group.category)+'" aria-labelledby="review-group-'+h(group.category)+'"><h2 tabindex="-1" id="review-group-'+h(group.category)+'">'+h(group.label)+'</h2>'+group.sections.map(section=>reviewSection(section,group.category,group.label)).join('')+'</section>';
...
function outputControls(position){
 return '<div class="review-controls b-output-controls no-print" data-output-controls="'+position+'">'+button(icon('pdf')+'<span>Ballot with Recommendations (PDF)</span>','ballot-pdf')+button(icon('print')+'<span>Print my Guide</span>','print')+'</div>';
}
function syncOutputControls(){
 for(const control of root.querySelectorAll('[data-action=ballot-pdf],[data-action=print]')){
  control.disabled=outputBusy;control.setAttribute('aria-busy',String(outputBusy));
 }
 for(const status of root.querySelectorAll('[data-output-status]')){status.textContent=outputMessage;status.hidden=!outputMessage;}
}
function outputSnapshot(){
 return {content,annotations:annotationStore.value.records,notes:notesStore.value.records.map(n=>({...n,sourceLinks:getNoteSources(activeBundle,n.id,n)})),areaLabel:area().displayLabel,isDemo:activeBundle.election.isDemo,electionLabel:activeBundle.election.title};
}
async function runGuideOutput(kind){
 if(outputBusy)return;
 let completedMessage='';
 outputBusy=true;outputMessage=kind==='pdf'?'Preparing your PDF…':'Preparing your guide…';syncOutputControls();announce(outputMessage);
 try{
  if(kind==='pdf'){
   const result=await makeBallotPdf(outputSnapshot()),url=URL.createObjectURL(result.blob);
   ...
   outputMessage=result.pageSize==='LEGAL'?'Use Legal paper (8.5 × 14 inches) for this PDF.':'';completedMessage='PDF downloaded.';
  }else{await printGuide(outputSnapshot());outputMessage='';completedMessage='Print dialog opened.';}
 }catch(error){outputMessage='Could not prepare this output. '+(error.message||'Please try again.');}
 finally{outputBusy=false;syncOutputControls();announce(outputMessage||completedMessage);scheduleBallotImport();}
}
// Native printing in My Guide uses the same current author snapshot.
window.addEventListener('beforeprint',()=>{if(screen==='review'&&hasActiveBallot()&&document.body.dataset.guidePrint!=='ready')prepareGuidePrint(outputSnapshot());});
window.addEventListener('afterprint',()=>{delete document.body.dataset.guidePrint;});
function reviewView(){
 ...
 const status='<p class="b-output-status no-print" data-output-status hidden></p>';
 return '<div class="review-content b-guide-review"><h1 class="sr-only" tabindex="-1">My guide</h1>'+navigation+outputControls('top')+status+'<div id="review-rows">'+(groups.length?rowReview(groups):'<p class="b-cheatsheet-empty" role="status">No recommendations have been added yet.</p>')+'</div>'+outputControls('bottom')+status+'</div>';
}
function originalAdminView(){
 const modified=changedGroups(original,content);
 return '<div class="race-layout"><h1 tabindex="-1">Admin</h1>'+ballotFilePanel()+'<p class="small">App '+h(APP_VERSION_LABEL)+' · Content revision '+revision+'</p><p class="muted">Edits stay in this browser. Export ballot JSON to save a complete copy.</p>'+(adminMessage||contentStore.warning?'<p class="notice" role="status">'+h(adminMessage||contentStore.warning)+'</p>':'')+button(icon('edit')+(admin?' Edit buttons are on':' Turn on Edit buttons'),'enable-admin',admin?'':'primary')+(admin?button('Leave Admin mode','leave-admin','quiet'):'')+annotationsAdminPanel()+notesAdminPanel()+'<section class="help-card"><h2>Edit ballot text and details</h2>'+button('Open the current section','start','primary')+'<label class="editor-field">Content group<select id="edit-group-select">'+Object.entries(collections).map(([group])=>'<optgroup label="'+h(groupLabel(group))+'">'+allGroups(content).filter(item=>item.group===group).map(item=>'<option value="'+item.group+':'+h(item.id)+'">'+h(item.label)+'</option>').join('')+'</optgroup>').join('')+'</select></label>'+button('Edit selected group','edit-selected')+'</section><section class="help-card"><h2>Edited locally</h2><p>'+modified.length+' text groups changed.</p>'+modified.map(item=>button(h(item.label),'edit','quiet compact','data-id="'+h(item.id)+'" data-group="'+item.group+'"')).join('')+button('Restore loaded text','restore-content')+(contentStore.previous?button('Review previous saved version','previous-content','quiet'):'')+'</section></div>';
}
...
function showCandidate(id){
 ...
 const media=get('media',candidate.portraitMediaId),photo=media?resolveContentUrl(media.path):'';
 openDialog(candidate.officialName,(text?.statusLabel?'<span class="tag example">'+h(text.statusLabel)+'</span>':'')+(photo?'<div class="portrait-line"><img src="'+h(photo)+'" alt="'+h(candidate.officialName)+'"><p>'+h(candidate.partyLabel||'')+'</p></div>':'')+detailBody(d,id)+editPill(text.id)+editPill(candidate.id,'candidate')+notePill(id)+sourcePanel());
}
...
function showRaceDetail(id){
 ...
 openDialog(race.officialTitle,'<p><strong>'+h(race.officialSelectionInstruction)+'</strong></p>'+detailBody(d,id)+editPill(race.editorialId)+notePill(id)+sourcePanel(race.sourceRefs[0].viewerPage));
}
...
async function saveDraft() {
  ...
  finally { busy = false; dialog.querySelectorAll('textarea,input,select').forEach(el => { el.disabled = false; }); scheduleBallotImport(); }
}
...
async function importFile(file) {
  ...
  } finally { busy = false; scheduleBallotImport(); }
}
...
const actions = {
  ...
  print: () => transition(() => runGuideOutput('print')),
  'ballot-pdf': () => transition(() => runGuideOutput('pdf')),
...
document.addEventListener('click', async (event) => { const section=event.target.closest('.b-ballot-section');if(ballotDialog.open&&section?.classList.contains('is-paper')&&!busy&&!event.target.closest('[data-section-nav]')){const target=event.target.closest('button,label')||event.target;prepareInteraction(section,target);}
...
document.addEventListener('change', (event) => {
  ...
  if(el.id==='ballot-file'){queueBallotImport(el.files[0]);el.value='';}
});
...
window.addEventListener('message',event=>{
 if(event.source!==window.parent||event.origin!==location.origin||event.data?.channel!=='ballot-preview-v1')return;
 const data=event.data;
 if(data.type==='ballot-import-handshake'){post('ballot-import-ready');return;}
 if(data.type==='import-ballot-file'){queueBallotImport(data.file,typeof data.requestId==='string'?data.requestId:null);return;}
 if(data.type==='ballot-import-error'){if(typeof data.error==='string')showImportError(data.error);return;}
 if(!model)return;
 if(data.type==='demo'&&['fresh','example','clear','restore'].includes(data.action))runDemo(data.action);
 if(data.type==='open-a')transition(()=>post('open-a-ready'));
 if(data.type==='print'&&hasActiveBallot())actions.print();
});
...
function ballotFilePanel(){
 return '<section class="help-card b-ballot-files" aria-labelledby="ballot-files-title"><h2 id="ballot-files-title">Ballot JSON</h2><p>One file holds this ballot, its opinion guide, and embedded files. Import replaces the current ballot and clears practice choices.</p><div class="b-file-actions">'+button('Download SCHEMAEXAMPLE.json','download-schema','')+button('Export ballot JSON','export-ballot','primary')+'</div><label class="editor-field" for="ballot-file">Import ballot JSON<input id="ballot-file" type="file" accept=".json,application/json" '+(importBusy?'disabled':'')+'></label><p class="small">Schema '+h(activeBundle?.schemaVersion||'1.0.0')+' · One city or township and precinct</p><p id="ballot-file-status" role="status" '+(importMessage?'':'hidden')+'>'+h(importMessage)+'</p></section>';
}
function syncImportStatus(){
 const status=document.querySelector('#ballot-file-status');if(status){status.textContent=importMessage;status.hidden=!importMessage;}
 document.querySelectorAll('#ballot-file,[data-action=export-ballot],[data-action=download-schema]').forEach(el=>el.disabled=importBusy);
}
function activateBallot(record,resources,{fresh=false,initial=false}={}){
 const adapted=adaptBallot(record.ballot,record.generation);
 if(!initial){finishBallot(false);closeMenu(false);if(dialog.open)finishClose();stopPaper();}
 const oldResources=activeResources;
 activeBundle=record.ballot;activeResources=resources;setAttachmentResolver(resources);
 seed=adapted.seed;original=adapted.original;settings=adapted.settings;
 areaCatalog=createAreaCatalog(adapted.catalog);activeAreaId=areaCatalog.value.defaultAreaId;draftAreaId=activeAreaId;
 activeBallot=areaCatalog.ballot(activeAreaId);areaKey='whatsonmyballot:'+new URL('../',import.meta.url).pathname+':'+seed.datasetId+':area';
 ...
 model.example=()=>{model.clearAll();for(const race of content.contests){if(race.candidateIds[0])model.choose(race.id,race.candidateIds[0],true);else if(race.optionIds[0])model.chooseOption(race.id,race.optionIds[0],true);}};
 notesStore=createNotesStore(adapted.notes,areaStorage,new URL('../',import.meta.url).pathname,activeBundle);notesStore.load();
 annotationStore=createAnnotationStore(adapted.annotations,areaStorage,new URL('../',import.meta.url).pathname);annotationStore.load();
 annotationExamples={options:[]};preferenceStorage=areaStorage;preferenceKey=annotationStore.key+':reader-view';
 showGuideMarks=true;onlyRecommendations=ballotHasRecommendations();
 try{const savedPrefs=JSON.parse(areaStorage?.getItem(preferenceKey));if(!fresh&&savedPrefs?.schemaVersion==='1.0.0'){showGuideMarks=savedPrefs.showGuideMarks!==false;onlyRecommendations=savedPrefs.onlyRecommendations!==false&&ballotHasRecommendations();}}catch{}
 variant={...(settings?.variants?.find(v=>v.id==='A5')||{}),id:'B.9A',showSelectionExplanation:false};
 selectedSectionId=content.contests[0].id;if(fresh)model.clearAll();else selectedSectionId=model.state.currentRaceId||selectedSectionId;
 model.visit(selectedSectionId);paperPage=currentRace().sourceRefs[0].viewerPage;
 paperState={zoom:1,x:0,y:0,mode:'extents'};tightSection=false;paperExpandedId=null;returnToReview=false;
 expanded.clear();opinionOverrides.clear();recoveryOffer=null;noteRecovery=null;annotationRecovery=null;
 outputMessage='';adminMessage=workspace?.warning||'';areaSaveWarning='';admin=false;
 if(fresh){screen='race';saveReaderPreferences();const url=new URL(location.href);url.search='';history.replaceState(null,'',url);}
 oldResources?.dispose();if(!initial)render({focus:true});
 document.body.dataset.ballotId=activeBundle.id;document.body.dataset.schemaVersion=activeBundle.schemaVersion;
}
// All file inputs and drops share this coordinator. Keep at most one pending file.
// A later pending file replaces an earlier one; an active transaction completes first.
function clearImportError(){
 importErrorMessage='';document.querySelector('#ballot-import-error')?.remove();
}
function presentImportError(){
 document.querySelector('#ballot-import-error')?.remove();if(!importErrorMessage)return;
 const surface=dialog.open?dialog:menuDialog.open?menuDialog:ballotDialog.open?ballotDialog:document.body;
 const notice=document.createElement('div');notice.id='ballot-import-error';notice.className='b-import-error';notice.setAttribute('role','alert');
 const text=document.createElement('span');text.textContent=importErrorMessage;notice.append(text);
 const close=document.createElement('button');close.type='button';close.setAttribute('aria-label','Dismiss import error');close.textContent='\u00d7';close.addEventListener('click',clearImportError);notice.append(close);surface.append(notice);
}
function showImportError(error){
 importErrorMessage=String(error?.message||error).slice(0,1200);presentImportError();announce(importErrorMessage);
}
function scheduleBallotImport(){
 if(importDrainTimer!==null)return;
 importDrainTimer=setTimeout(()=>{importDrainTimer=null;drainBallotImport();},0);
}
function queueBallotImport(file,requestId=null){
 if(!file)return;
 if(!(file instanceof File)){
  const error='Drop one ballot JSON file.';showImportError(error);post('ballot-import-result',{requestId,ok:false,error});return;
 }
 pendingBallotImport={file,requestId};clearImportError();scheduleBallotImport();
}
function drainBallotImport(){
 if(!appInitialized||busy||outputBusy||importBusy||!pendingBallotImport)return;
 const pending=pendingBallotImport;pendingBallotImport=null;
 void importBallotFile(pending.file,pending.requestId);
}
async function importBallotFile(file,requestId=null){
 importBusy=true;busy=true;importMessage='Loading ballot\u2026';syncImportStatus();let prepared=null;
 const editorFields=[...dialog.querySelectorAll('textarea,input,select')].map(el=>[el,el.disabled]);
 editorFields.forEach(([el])=>el.disabled=true);
 try{
  if(file.size>MAX_BALLOT_BYTES)throw Error('The ballot JSON is too large.');
  const ballot=await parseBallot(await file.text());prepared=await prepareResources(ballot.attachments||[]);
  const test=adaptBallot(ballot,'preflight');
  validateEnvelope(test.seed,test.seed,test.original);createNotesStore(test.notes,null,'preflight',ballot);createAnnotationStore(test.annotations,null,'preflight');createAreaCatalog(test.catalog);resolveBallotLayout(test.original);
  if(!workspace)workspace=await openWorkspace({storage:areaStorage});
  const record=await workspace.replace(ballot);
  clearImportError();activateBallot(record,prepared,{fresh:true});prepared=null;
  document.body.dataset.ready='true';importMessage='';post('ready',{id:variant.id});
  post('ballot-import-result',{requestId,ok:true});
 }catch(error){
  prepared?.dispose();importMessage='Could not import: '+error.message;syncImportStatus();showImportError(importMessage);
  post('ballot-import-result',{requestId,ok:false,error:importMessage});
 }finally{
  editorFields.forEach(([el,disabled])=>{if(el.isConnected)el.disabled=disabled;});
  importBusy=false;busy=false;syncImportStatus();scheduleBallotImport();
 }
}
async function downloadSchemaExample(){
 const response=await fetch(new URL('../SCHEMAEXAMPLE.json',import.meta.url));if(!response.ok)throw Error('The schema example could not load.');
 const value=await response.json();saveJson(value,'SCHEMAEXAMPLE.json');announce('Schema example downloaded.');
}
function exportCurrentBallot(){
 const value=exportBallot(activeBundle,{content,notes:notesStore.value,annotations:annotationStore.value,baseline:original});
 const result=validateBallot(value);if(!result.ok)throw Error(result.errors.map(e=>typeof e==='string'?e:e.message).join('; '));
 saveJson(value,(activeBundle.id||'ballot')+'.json');announce('Ballot JSON downloaded.');
}
async function init(){
 try{
  try{areaStorage=window.localStorage;}catch{}
  workspace=await openWorkspace({storage:areaStorage});
  let record=await workspace.load();
  if(!record){const response=await fetch(new URL('../data/default-ballot.json',import.meta.url));if(!response.ok)throw Error('The default ballot could not load.');record=await workspace.replace(await parseBallot(await response.text()));}
  const ballot=await parseBallot(JSON.stringify(record.ballot)),resources=await prepareResources(ballot.attachments||[]);
  activateBallot({...record,ballot},resources,{initial:true});
  const params=new URLSearchParams(location.search);if(['race','review','admin','district'].includes(params.get('screen')))screen=params.get('screen');
  if(params.get('race')&&content.contests.some(r=>r.id===params.get('race'))){selectedSectionId=params.get('race');model.visit(selectedSectionId);}
  if(params.get('demo')==='fresh'){model.clearAll();selectedSectionId=content.contests[0].id;model.visit(selectedSectionId);screen='race';onlyRecommendations=ballotHasRecommendations();showGuideMarks=true;saveReaderPreferences();}
  render();document.body.dataset.ready='true';post('ready',{id:variant.id});offerRecovery();
 }catch(error){
  root.innerHTML='<main class="loading"><h1>The ballot could not load.</h1><p role="alert">'+h(error.message)+'</p><label class="editor-field">Import ballot JSON<input type="file" id="ballot-file" accept=".json,application/json"></label><p id="ballot-file-status" role="status"></p></main>';console.error(error);
 }finally{appInitialized=true;scheduleBallotImport();}
}
...
function runDemo(action){
 ...
 confirmAction(action==='fresh'?'Fresh Start?':'Load example choices?',action==='fresh'?'Clear personal choices and return to the first section? The sample opinion guide and saved text stay as they are.':'Replace personal choices with the bundled sample choices?',action==='fresh'?'Start fresh':'Load example',()=>{
  finishBallot(false);if(action==='fresh'){model.clearAll();onlyRecommendations=ballotHasRecommendations();showGuideMarks=true;saveReaderPreferences();selectedSectionId=content.contests[0].id;paperPage=1;screen='race';}else{model.example();screen='review';}render({focus:true});
 });
}
...
Object.assign(actions,{
 ...
 'create-ballot':()=>openDialog('Create ballot','<p>This feature is planned. No ballot is created in B.9A.</p><p>Choose an area with ballot data to try the existing local editor.</p>',button('Close','close-dialog')),
...
Object.assign(actions,{'download-schema':downloadSchemaExample,'export-ballot':()=>transition(exportCurrentBallot)});
installFileDrop({onFile:file=>queueBallotImport(file),onError:showImportError});
post('ballot-import-ready');
window.addEventListener('pagehide',event=>{if(!event.persisted){clearTimeout(importDrainTimer);activeResources?.dispose();workspace?.close();}});
init();
