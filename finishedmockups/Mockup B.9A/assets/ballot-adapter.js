// Pure bridge from the portable B.9A ballot to the established H editor/view model.
...
export function adaptBallot(ballot, generation) {
  const datasetId = 'b9a-workspace-' + String(generation).replace(/[^a-zA-Z0-9-]/g, '-').toLowerCase();
...
// attachments, then apply only changes saved through the B.9A author editors.
