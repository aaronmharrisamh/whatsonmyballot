...
export function mountPaper(viewport,paper,saved,onChange,onView=()=>{},onSnap=()=>{}){
 ...
 function paint(){
  ...
  Object.assign(viewport.dataset,{zoom:state.zoom.toFixed(4),panX:state.x.toFixed(2),panY:state.y.toFixed(2),scale:scale().toFixed(5),mode:state.mode,zoomView:state.zoomView,animating:String(Boolean(tween||wheelMotion))});
 ...
 function tick(now){
  ...
  if(wheelMotion){
   const motion=wheelMotion,elapsed=Math.min(48,Math.max(1,now-motion.time));
   motion.time=now;
   const range=limits(),target=clamp(motion.zoom,range.min,range.max),difference=Math.log(target/state.zoom);
   // One time-based response follows the wheel target without restarting a
   // tween for each event. Each frame holds the content beneath the pointer.
   const zoom=Math.abs(difference)<.0005?target:state.zoom*Math.exp(difference*(1-Math.exp(-elapsed/55)));
   const ratio=zoom/state.zoom;
   state.x=motion.point.x-(motion.point.x-state.x)*ratio;
   state.y=motion.point.y-(motion.point.y-state.y)*ratio;state.zoom=zoom;
   if(zoom===target)wheelMotion=null;
  }
  paint();if(tween||wheelMotion)frame=requestAnimationFrame(tick);
 }
 function queue(){if(!frame&&!disposed)frame=requestAnimationFrame(tick);}
 function stop(){tween=null;wheelMotion=null;cancelAnimationFrame(frame);frame=0;clearTimeout(wheelTimer);}
 ...
 function columnAtCenter(){
  const x=(vw/2-state.x)/scale();
  let nearest=null,distance=Infinity;
  for(const column of paper.querySelectorAll('.paper-column')){
   if(!column.querySelector('.b-ballot-section'))continue;
   const p=position(column),d=Math.max(p.left-x,0,x-p.left-p.width);
   if(d<distance){nearest=column;distance=d;}
  }
  return distance*scale()<=vw*.5?nearest:focused?.closest('.paper-column');
 }
 function recoverColumnStart(element){
  if(!element?.isConnected)return false;
  const column=element.closest('.paper-column')||element,p=position(column),top=state.y+p.top*scale();
  if(top<=vh*.5)return false;
  const first=column.matches('.b-ballot-section')?column:column.querySelector('.b-ballot-section');
  if(first&&first!==focused){onSnap(first.dataset.raceId);focused=first;}
  const zoom=state.zoom;
  // Once more than half the viewport is blank, restore the start and its Back
  // button without changing zoom, matching the existing column-end recovery.
  animate(()=>{
   const box=position(column),sc=fit*zoom;
   return{x:(vw-box.width*sc)/2-box.left*sc,y:12-box.top*sc,zoom};
  },'free',true,420);
  return true;
 }
 ...
 function snap(){
  ...
  const column=columnAtCenter();
  if(recoverColumnStart(column)||recoverColumnEnd(column))return;
 ...
 function down(e){
  ...
  if(!pointers.size){moved=false;pinched=false;suppressUntil=0;start=local(e);stop();paint();}
  ...
  if(pointers.size>1){stop();for(const id of pointers.keys())viewport.setPointerCapture(id);}
 }
 function movePointer(e){
  if(!pointers.has(e.pointerId))return;
  const point=local(e);pointers.set(e.pointerId,point);const g=geometry();
  if(moved||pointers.size>1||Math.hypot(point.x-start.x,point.y-start.y)>6){
 ...
 function up(e){
  ...
  const g=geometry();lastCenter=g.center;lastDistance=g.distance;start=g.center;
  if(moved||pinched)suppressUntil=performance.now()+400;
  if(!pointers.size){viewport.classList.remove('dragging');if(moved&&!pinched&&e.type==='pointerup')snap();}
 }
 ...
 function wheel(e){
  ...
  // Read deltaMode before deltaY: line and page units differ from pixels.
  const factor=e.deltaMode===1?18:e.deltaMode===2?vh:1,delta=e.deltaY*factor;
  if(!snapEnabled||e.ctrlKey){
   if(!delta)return;
   const point=local(e),range=limits(),direction=Math.sign(delta),now=performance.now();
   // Reverse from the rendered camera, not a still-pending target. Input at a
   // limit is consumed, so reversing direction never has a dead zone.
   const continuing=wheelMotion&&wheelMotion.direction===direction;
   const base=continuing?wheelMotion.zoom:state.zoom;
   const zoom=clamp(base*Math.exp(clamp(-delta*(e.ctrlKey?.008:.0025),-.65,.65)),range.min,range.max);
   if(!wheelMotion)stop();
   state.mode='free';state.zoomView='custom';
   if(reduced()){zoomTo(zoom,point,false);return;}
   wheelMotion={zoom,point,direction,time:wheelMotion?.time||now};
   viewport.dataset.animating='true';queue();return;
  }
  stop();state.mode='free';state.x-=e.deltaX*factor;state.y-=delta;paint();
 ...
 for(const [name,handler]of [['pointerdown',down],['pointermove',movePointer],['pointerup',up],['pointercancel',up],['lostpointercapture',up]])viewport.addEventListener(name,handler);
 ...
 const paperObserver=new ResizeObserver(()=>{
  ...
  if(!tween&&!wheelMotion&&state.mode==='extents')extents(false);else queue();
 ...
 return{zoomTo,zoomBy,extents,reset:extents,move,panToElement,toggleZoom,focusElement,focusNearestSection,ensureComfortable,captureAnchor,holdAnchor,
  ...
  reflow:queue,snapNow:snap,setSnap(value){const next=Boolean(value);if(next!==snapEnabled&&wheelMotion){stop();paint();}snapEnabled=next;if(!snapEnabled&&state.mode==='section'){stop();state.mode='free';paint();}},
  selected(element){focused=element;queue();},
  destroy(){disposed=true;stop();observer.disconnect();paperObserver.disconnect();
   for(const [name,handler]of [['pointerdown',down],['pointermove',movePointer],['pointerup',up],['pointercancel',up],['lostpointercapture',up]])viewport.removeEventListener(name,handler);
