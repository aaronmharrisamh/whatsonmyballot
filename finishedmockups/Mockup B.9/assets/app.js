...
function raceView(){
 const r=currentRace();
 return '<div class="b-section-view"><div class="b-section-intro"><div><h1 tabindex="-1">Voting Guide for <span>'+h(area().displayLabel)+'</span></h1><p class="b-demo-label">'+(activeBundle.election.isDemo?'Fictional sample · ':'')+'Opinion guide</p></div></div><div id="guide-progress">'+renderProgress(content,selectedSectionId,id=>model.count(id))+'</div><div class="b-guide-options">'+recommendationsToggle('readable')+'</div>'+/* Upper Back chip hidden in B.9; candidate for complete removal later: adjacentChip(-1). */'<div class="b-readable-paper">'+ballotSection(r)+'</div>'+adjacentChip(1)+'<div class="b-adjacent b-back-start" '+(ballotUI.showBackToStart?'':'hidden')+'>'+button(icon('back')+'<span>Back to Start</span>','back-to-start','b-step-chip')+'</div><div class="selection-message" id="selection-feedback" role="status" hidden></div></div>';
}
...
function render({focus=false,keepScroll=false}={}){
 ...
 document.body.dataset.design='B.9';document.body.dataset.screen=screen;document.body.dataset.areaAvailable=String(available);
...
async function runGuideOutput(kind){
 ...
 finally{outputBusy=false;syncOutputControls();announce(outputMessage||completedMessage);}
}
...
document.addEventListener('change', (event) => {
  ...
  if(el.id==='ballot-file'){importBallotFile(el.files[0]);el.value='';}
});
...
function activateBallot(record,resources,{fresh=false,initial=false}={}){
 ...
 showGuideMarks=true;onlyRecommendations=true;
 try{const savedPrefs=JSON.parse(areaStorage?.getItem(preferenceKey));if(!fresh&&savedPrefs?.schemaVersion==='1.0.0'){showGuideMarks=savedPrefs.showGuideMarks!==false;onlyRecommendations=savedPrefs.onlyRecommendations!==false;}}catch{}
 variant={...(settings?.variants?.find(v=>v.id==='A5')||{}),id:'B.9',showSelectionExplanation:false};
...
async function importBallotFile(file){
 if(!file||importBusy||outputBusy)return;
 ...
 try{
  ...
  activateBallot(record,prepared,{fresh:true});prepared=null;
  importMessage='Loaded '+activeBundle.title+'.';announce(importMessage);
 }catch(error){prepared?.dispose();importMessage='Could not import: '+error.message;syncImportStatus();announce(importMessage);}
 finally{importBusy=false;busy=false;syncImportStatus();}
}
...
async function init(){
 try{
  ...
  if(params.get('demo')==='fresh'){model.clearAll();selectedSectionId=content.contests[0].id;model.visit(selectedSectionId);screen='race';onlyRecommendations=true;showGuideMarks=true;saveReaderPreferences();}
...
Object.assign(actions,{
 ...
 'create-ballot':()=>openDialog('Create ballot','<p>This feature is planned. No ballot is created in B.9.</p><p>Choose an area with ballot data to try the existing local editor.</p>',button('Close','close-dialog')),
...
window.addEventListener('pagehide',event=>{if(!event.persisted){activeResources?.dispose();workspace?.close();}});
init();
