// One active B.9 ballot. The full portable bundle is an atomic IndexedDB record.
...
export async function openWorkspace({ indexedDB = globalThis.indexedDB, storage = globalThis.localStorage, scope = new URL('../', import.meta.url).pathname } = {}) {
  ...
  const db = await new Promise((resolve, reject) => {
    const request = indexedDB.open('whatsonmyballot-b9:' + scope, 1);
    request.onupgradeneeded = () => request.result.createObjectStore('workspace');
    request.onsuccess = () => resolve(request.result);
    request.onerror = () => reject(new Error('The ballot workspace could not be opened.'));
    request.onblocked = () => reject(new Error('Close another B.9 tab and try again.'));
  });
  ...
  async function load() {
    return new Promise((resolve, reject) => {
      ...
      tx.oncomplete = () => {
        if (result && (result.format !== 'whatsonmyballot-b9-workspace' || typeof result.generation !== 'string' || !result.ballot)) return reject(new Error('The saved ballot workspace is unreadable.'));
  ...
  async function replace(ballot) {
    ...
    const next = { format: 'whatsonmyballot-b9-workspace', schemaVersion: '1.0.0', generation, ballot: copy(ballot) };
    ...
    await new Promise((resolve, reject) => {
      ...
      tx.onerror = tx.onabort = () => reject(new Error(conflict ? 'Another B.9 tab imported a ballot. Reload before importing again.' : 'The new ballot could not be saved. The current ballot is unchanged.'));
    });
    currentGeneration = generation;
    // B.9-generated dataset IDs are namespaced independently of all old mockups.
    try {
      const remove = [];
      for (let i = 0; i < storage?.length; i++) {
        const key = storage.key(i);
        if (key?.startsWith('whatsonmyballot:') && (key.includes(':b9-workspace-') || key.includes('/Mockup%20B.9/') || key.includes('/Mockup B.9/'))) remove.push(key);
      }
