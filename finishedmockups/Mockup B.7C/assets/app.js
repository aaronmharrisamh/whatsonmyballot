...
function sourcePanel(page=null){
 return '<div class="source-panel"><strong>Original ballot layout reference</strong><p>Akron Township, Precinct 1AF · November 5, 2024'+(page?' · Page '+page:'')+'</p><p class="small">B.7C replaces the names with fictional entries. Its random guide marks are not on this PDF.</p><a href="'+h(pdfPath())+(page?'#page='+page:'')+'" target="_blank" rel="noopener noreferrer">'+icon('pdf')+' Open original sample PDF '+icon('source')+'</a></div>';
}
...
function raceView(){
 const r=currentRace();
 return '<div class="b-section-view"><div class="b-section-intro"><div><h1 tabindex="-1">Voting Guide for <span>'+h(area().displayLabel)+'</span></h1><p class="b-demo-label">Fictional options · Sample opinion guide</p></div></div><div id="guide-progress">'+renderProgress(content.contests,selectedSectionId,id=>model.count(id),activeBallot.pages)+'</div><div class="b-guide-options">'+recommendationsToggle('readable')+'</div>'+/* Upper Back chip hidden in B.7C; candidate for complete removal later: adjacentChip(-1). */'<div class="b-readable-paper">'+ballotSection(r)+'</div>'+adjacentChip(1)+'<div class="b-adjacent b-back-start" '+(ballotUI.showBackToStart?'':'hidden')+'>'+button(icon('back')+'<span>Back to Start</span>','back-to-start','b-step-chip')+'</div><div class="selection-message" id="selection-feedback" role="status" hidden></div></div>';
}
...
function render({focus=false,keepScroll=false}={}){
 ...
 document.body.dataset.design='B.7C';document.body.dataset.screen=screen;document.body.dataset.areaAvailable=String(available);
...
async function init() {
 try{
  ...
  selectedSectionId=content.contests[0].id;variant={...settings.variants.find(e=>e.id==='A5'),id:'B.7C',showSelectionExplanation:false};
...
Object.assign(actions,{
 ...
 'create-ballot':()=>openDialog('Create ballot','<p>This feature is planned. No ballot is created in B.7C.</p><p>Choose an area with ballot data to try the existing local editor.</p>',button('Close','close-dialog')),
