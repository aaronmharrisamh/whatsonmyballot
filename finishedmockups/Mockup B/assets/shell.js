...
function notes(){open('B1 · The ballot comes first','<span class="eyebrow">YOUR VOICE. YOUR VOTE.</span><h3>A familiar shape. Two ways to read.</h3><p>The complete virtual ballot opens first. Zoom and move around, or read one section at a comfortable size.</p><p>Chevrons open a short explanation. The magnifying glass opens a longer reading window. Both sit beside the ballot content.</p><p>The palette takes its cues from the District 9 website’s muted flag and warm neutrals, with navy and small red accents.</p><p>Choices and saved text are shared with A1–A5 at this site. Profiles remain labeled examples.</p><p><a href="../../sharedrefs/mockup-b/README.md" target="_blank" rel="noopener noreferrer">B1 review guide</a></p><p>'+h(APP_VERSION_LABEL)+' · Historical November 2024 sample</p>');}
...
document.querySelector('#compare-button').innerHTML=icon('grid')+'<span>A designs</span>';
...
window.addEventListener('message',event=>{if(event.source===iframe.contentWindow&&event.origin===location.origin&&event.data?.channel==='ballot-preview-v1'&&event.data.type==='open-a-ready')location.href='../Mockup%20A/index.html';});
