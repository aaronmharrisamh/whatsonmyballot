...
export function createIdleHint(doc=document){
 ...
 function schedule(){
  clearTimeout(timer);
  if(!element||!active||doc.hidden||doc.querySelector('dialog[open]'))return;
  const delay=delays[Math.min(shown,2)];element.dataset.nextDelay=String(delay);
  timer=setTimeout(()=>{if(active&&!doc.hidden&&!doc.querySelector('dialog[open]')){element.classList.add('is-visible');shown++;element.dataset.shown=String(shown);}},delay);
 }
