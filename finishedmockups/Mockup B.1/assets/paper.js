...
export function mountPaper(viewport,paper,saved,onChange){
 ...
 function interrupt(){
  ...
  paper.style.transition='none';state.mode='free';
 }
 ...
 const paperObserver=new ResizeObserver(()=>{
  ...
  if(state.mode==='extents')extents(false);else paint();
 });paperObserver.observe(paper);
 return {zoomTo,extents,reset:extents,move,focusElement,reflow:()=>paint(),
  selected(element){focused=element;},
  destroy(){disposed=true;observer.disconnect();paperObserver.disconnect();
