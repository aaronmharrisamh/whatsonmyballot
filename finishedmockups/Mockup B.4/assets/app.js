...
function paintFocus(animate=false){
 for(const n of choiceNodes('.b-ballot-section')){
  ...
  for(const tool of n.querySelectorAll('.b-row-tools,.b-clear-chip,.b-annotation-edit'))tool.inert=!selected;
...
function prepareInteraction(section,target){
 ...
 paperController?.ensureComfortable(section,target,tightSection?1:.75);
}
...
function showOpinion(el){
 ...
 opinionOverrides.set(id,!opinionOpen(id));
 syncOpinionPanels(id);paperController?.reflow();
}
...
const guideCheck='<svg class="b-guide-glyph" viewBox="0 0 32 32" role="img" aria-label="THIS — guide mark"><path d="m5 16 7 7L27 8" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/></svg>';
const guideOr='<svg class="b-guide-glyph" viewBox="0 0 40 32" role="img" aria-label="OR — another guide option"><rect x="2" y="5" width="36" height="22" rx="11" fill="none" stroke="currentColor" stroke-width="2"/><path d="M13 11c-5 0-5 10 0 10s5-10 0-10Zm9 10V11h3c5 0 5 5 0 5h-3m3 0 4 5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
let annotationStore,annotationRecovery=null,annotationExamples,showGuideMarks=true,tightSection=false,preferenceStorage=null,preferenceKey='';
...
function opinionPanel(id,prefix){
 return '<aside class="b-inline-opinion" id="'+prefix+'-opinion-'+h(id)+'" data-opinion-panel="'+h(id)+'" '+annotationStyle(annotationStore.get(id))+' '+(opinionOpen(id)?'':'hidden')+' aria-label="Opinion on '+h(notesStore.get(id).label)+'">'+opinionBody(id)+'</aside>';
}
function syncOpinionPanels(id=null){
 choiceNodes('[data-opinion-panel]').forEach(panel=>{
  const key=panel.dataset.opinionPanel;if(id&&key!==id)return;
  panel.hidden=!opinionOpen(key);panel.innerHTML=opinionBody(key);
 });
...
function raceView(){
 const r=currentRace();
 return '<div class="b-section-view"><div class="b-section-intro"><div><h1 tabindex="-1">Voting Guide for <span>'+h(area().municipality||'Akron Township')+' · '+h('Precinct '+(area().precinct||'1AF'))+'</span></h1></div></div><div id="guide-progress">'+renderProgress(content.contests,selectedSectionId,id=>model.count(id))+'</div><div class="b-guide-options"><label class="b-guide-toggle"><input type="checkbox" id="show-guide-marks" '+(showGuideMarks?'checked':'')+'>Show guide marks</label><span class="b-guide-key" aria-label="Guide symbols: check means THIS; OR means another option">'+guideGlyph('this')+'<span>THIS</span>'+guideGlyph('or')+'</span></div><div class="b-readable-paper">'+ballotSection(r)+'</div><div class="selection-message" id="selection-feedback" role="status" hidden></div></div>';
}
function dockMap(){
 return button(renderMinimap(content.contests,selectedSectionId)+'<span>Ballot View</span>','open-ballot','b-minimap b-dock-map','aria-label="Open Ballot View. Section '+(sectionIndex()+1)+' of '+content.contests.length+'" aria-haspopup="dialog" aria-expanded="'+ballotDialog.open+'" aria-controls="ballot-dialog"');
}
function updateDockMap(){
 const map=root.querySelector('.b-dock-map');
 if(map){map.innerHTML=renderMinimap(content.contests,selectedSectionId)+'<span>Ballot View</span>';map.setAttribute('aria-label','Open Ballot View. Section '+(sectionIndex()+1)+' of '+content.contests.length);map.setAttribute('aria-expanded',String(ballotDialog.open));}
 syncProgress();
}
...
function helpView(){
 return '<div class="race-layout b-help"><h1 tabindex="-1">Keep your place.</h1><section class="help-card"><h2>Read one section</h2><p>Back and Next move through all 28 sections. The progress line follows their relative size. A check means that a section has at least one saved choice; it can still have space for more.</p><p>Ballot View opens the map in its own window. Close it with X to return to your selected section.</p></section><section class="help-card"><h2>Your marks and guide marks</h2><p>A filled circle is your choice. A faint check means THIS in the editor’s guide. OR means another option. Guide marks never fill your own circles.</p><p>Turn off Show guide marks to hide highlights, guide symbols, and automatic opinions. You can still open any Opinion chip.</p><p>The demo has a separate fictional example in Admin. No guide marks are assigned to real candidates in the original sample.</p></section><section class="help-card"><h2>Move around the ballot</h2><p>Pinch to zoom and drag to move. Extents shows the current page. Prev and Next find a section. Check Section to fit its full width; leave it unchecked for more space around it.</p><p>Slide the right-hand zoom control for a closer view. On a computer, use Ctrl and the scroll wheel to zoom. Arrow keys move the focused canvas. Home shows the full page. Escape closes the top window.</p></section><section class="help-card"><h2>Read the notes</h2><p>Opinion opens a note below its row. The magnifying glass opens more information. A write-in name counts toward the choice limit.</p></section><section class="help-card"><h2>About this sample</h2><p>This is the historical November 2024 Akron Township 1AF sample. Profiles are examples. Demo notes have a fictional author.</p>'+sourcePanel()+'</section><p class="small muted">'+h(statusLabel())+' App '+h(APP_VERSION_LABEL)+'</p>'+button('Clear personal choices','clear-all')+'</div>';
}
...
function render({focus=false,keepScroll=false}={}){
 ...
 document.body.dataset.design='B.4';document.body.dataset.screen=screen;
...
function updatePaperControls(){
 ...
 if(slider){
  const value=Math.max(0,Math.min(100,Math.log(Math.max(.01,paperState.zoom)/.05)/Math.log(320)*100));
  slider.value=String(value);slider.setAttribute('aria-valuetext',Math.round(paperState.zoom*100)+' percent of page width');
 }
}
function updateViewedSection(){}
...
document.addEventListener('change', (event) => {
  ...
  if(el.id==='section-zoom'){tightSection=el.checked;paintFocus(true);paperController?.focusElement(ballotDialog.querySelector('#paper-'+CSS.escape(selectedSectionId)),true,tightSection?1:.75);}
...
document.addEventListener('input', (event) => {
  ...
  if(el.id==='ballot-zoom')paperController?.zoomTo(.05*Math.pow(320,Number(el.value)/100),undefined,false);
  if (el.matches('[data-edit-field]')) updateDraft();
});
...
async function init() {
  try {
    ...
    const params = new URLSearchParams(location.search); variant = { ...settings.variants.find(entry => entry.id === 'A5'), id: 'B.4', showSelectionExplanation: false };
