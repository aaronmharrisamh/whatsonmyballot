...
function sourcePanel(page=null){
 return '<div class="source-panel"><strong>Original ballot layout reference</strong><p>Akron Township, Precinct 1AF · November 5, 2024'+(page?' · Page '+page:'')+'</p><p class="small">B.6 replaces the names with fictional entries. Its random guide marks are not on this PDF.</p><a href="'+h(pdfPath())+(page?'#page='+page:'')+'" target="_blank" rel="noopener noreferrer">'+icon('pdf')+' Open original sample PDF '+icon('source')+'</a></div>';
}
...
const guideCheck='<svg class="b-guide-glyph" viewBox="0 0 48 32" role="img" aria-label="THIS — sample guide mark"><ellipse cx="24" cy="16" rx="18" ry="10" fill="none" stroke="currentColor" stroke-width="2" class="b-guide-outline"/><path d="m15 16 6 6 12-12" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>';
const guideOr='<svg class="b-guide-glyph" viewBox="0 0 48 32" role="img" aria-label="OR — another sample option"><ellipse cx="24" cy="16" rx="18" ry="10" fill="none" stroke="currentColor" stroke-width="2" class="b-guide-outline"/><path d="M18 10c-6 0-6 12 0 12s6-12 0-12Zm10 12V10h3c6 0 6 6 0 6h-3m3 0 5 6" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
function opinionBody(id){
 ...
 const label=marked?(a.mark==='this'?' — Choose This':' — Optional'):'';
 return '<div class="b-opinion-heading">'+scaleIcon+'<span>Opinion'+label+'</span></div><p class="b-inline-author">'+h(n.author||'No author added')+'</p><div class="b-inline-copy">'+(n.opinion.trim()?n.opinion.split(/\n{2,}/).map(p=>'<p>'+h(p)+'</p>').join(''):'<p>No opinion added.</p>')+'</div>'+(n.sourceUrl?'<a href="'+h(n.sourceUrl)+'" target="_blank" rel="noopener noreferrer">Opinion source '+icon('source')+'</a>':'')+notePill(n.id);
}
...
function changeGuideVisibility(visible){
 ...
 syncMarks();syncOpinionPanels();paperController?.reflow();
...
function raceView(){
 const r=currentRace();
 return '<div class="b-section-view"><div class="b-section-intro"><div><h1 tabindex="-1">Voting Guide for <span>'+h(area().displayLabel)+'</span></h1><p class="b-demo-label">Fictional options · Sample opinion guide</p></div></div><div id="guide-progress">'+renderProgress(content.contests,selectedSectionId,id=>model.count(id),activeBallot.pages)+'</div><div class="b-guide-options"><label class="b-guide-toggle"><input type="checkbox" role="switch" id="show-guide-marks" '+(showGuideMarks?'checked':'')+'><span class="b-switch-track" aria-hidden="true"></span><span>Show opinion guide</span></label></div>'+adjacentChip(-1)+'<div class="b-readable-paper">'+ballotSection(r)+'</div>'+adjacentChip(1)+'<div class="selection-message" id="selection-feedback" role="status" hidden></div></div>';
}
...
function helpView(){
 return '<div class="race-layout b-help"><h1 tabindex="-1">Explore the example.</h1><section class="help-card"><h2>Choose your area</h2><p>Open District. Choose an area, then press Use this area. A small ballot icon means ballot information is available.</p><p>Known area names appear as text. Only fields with more than one option use a dropdown. Areas without data keep District and Admin available.</p></section><section class="help-card"><h2>Read the guide</h2><p>Your first visit starts with no personal choices. A steady guide check or OR appears inside an oval with a soft moving outline. A filled oval means you made a choice.</p><p>Show opinion guide controls the sample highlights and automatic opinions. Choose This and Optional refer to the fictional guide. You can open other opinions by hand.</p></section><section class="help-card"><h2>Move around the ballot</h2><p>View and the small map open Ballot Viewer at the whole-page view. Zoom switches between the page and the current section. Plus, minus, the slider, and pinch adjust zoom.</p><p>Next and Prev move to the section top. Snap recenters after a drag without changing zoom. You can pan beyond the paper. Tap the backdrop or X to close the viewer.</p><p>All rows have small information buttons. Sample guide opinions are open on all highlighted rows. The edge arrow returns to your selected section.</p></section><section class="help-card"><h2>Try the demo</h2><p>Menu contains Demo controls, Admin, and Help. Fresh Start clears personal choices but keeps the fictional guide and saved text. Ballot creation is a placeholder. Local edits stay in this browser.</p></section>'+(hasActiveBallot()?sourcePanel():'')+'<p class="small muted">App '+h(APP_VERSION_LABEL)+'</p></div>';
}
...
function render({focus=false,keepScroll=false}={}){
 ...
 document.body.dataset.design='B.6';document.body.dataset.screen=screen;document.body.dataset.areaAvailable=String(available);
...
async function init() {
 try{
  ...
  try{const s=JSON.parse(areaStorage?.getItem(preferenceKey));if(s?.schemaVersion==='1.0.0'&&typeof s.showGuideMarks==='boolean')showGuideMarks=s.showGuideMarks;}catch{}
  selectedSectionId=content.contests[0].id;variant={...settings.variants.find(e=>e.id==='A5'),id:'B.6',showSelectionExplanation:false};
  ...
  if(params.get('demo')==='fresh'){model.clearAll();selectedSectionId=content.contests[0].id;screen='race';const url=new URL(location.href);url.searchParams.delete('demo');history.replaceState(null,'',url);}
...
function runDemo(action){
 ...
 confirmAction(action==='fresh'?'Fresh Start?':'Load example choices?',action==='fresh'?'Clear personal choices and return to the first section? The sample opinion guide and saved text stay as they are.':'Replace personal choices with the bundled sample choices?',action==='fresh'?'Start fresh':'Load example',()=>{
  finishBallot(false);if(action==='fresh'){model.clearAll();selectedSectionId=content.contests[0].id;paperPage=1;screen='race';}else{model.example();screen='review';}render({focus:true});
 });
}
...
Object.assign(actions,{
 ...
 'create-ballot':()=>openDialog('Create ballot','<p>This feature is planned. No ballot is created in B.6.</p><p>Choose an area with ballot data to try the existing local editor.</p>',button('Close','close-dialog')),
